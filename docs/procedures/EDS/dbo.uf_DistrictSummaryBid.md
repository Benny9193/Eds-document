# Function: table-valued: `dbo.uf_DistrictSummaryBid`

_Generated on 2026-05-04T13:04:00.544Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `uf_DistrictSummaryBid` |
| Kind | Function (table-valued) |
| sys.objects.type | `TF` (SQL_TABLE_VALUED_FUNCTION) |
| Created | 2006-05-31 10:28:37 |
| Modified | 2022-05-19 11:23:11 |
| Encrypted | no |
...
| Caller | Type |
|--------|------|
| `dbo.rs_DistrictSummary` | VIEW |
| `dbo.uf_AwardLetterBid` | SQL_TABLE_VALUED_FUNCTION |
| `dbo.uf_AwardLetterBid_Orig` | SQL_TABLE_VALUED_FUNCTION |
| `dbo.uf_AwardLetterBid1` | SQL_TABLE_VALUED_FUNCTION |
| `dbo.uf_DistrictSummaryVendorsBid` | SQL_TABLE_VALUED_FUNCTION |

## Definition

```sql
CREATE   function [dbo].[uf_DistrictSummaryBid] (@pRSId int, @pCategoryId int, @pBidHeaderId int)
...
begin
  insert @RetTable (ItemId, BidPrice, GrossPrice, Quantity, VendorId, VendorItemCode, Alternate, DistrictId, ListId, CategoryName, DistrictName, ItemCode, SortSeq, Description, UnitCode, VendorCode, UnitPrice, ExtendedPrice, DiscountRate, UseGrossPrices, CategoryId, PricePlanId, AwardId, BudgetId, BidHeaderId)
    select dbo.Detail.ItemId,
           ...
           sum(case isnull(dbo.Detail.UseGrossPrices,0) when 0 then round(Detail.BidPrice,2) * Detail.Quantity else round(Detail.GrossPrice,2) * Detail.Quantity end + coalesce(case when Detail.AdditionalShipping = 1 then Detail.ShippingCost else 0 end,0)) ExtendedPrice,
           ...
           case isnull(dbo.Detail.ItemMustBeBid,0) when 1 then case isnull(dbo.Detail.BidHeaderId,0) when 0 then null else dbo.Detail.BidHeaderId end else case isnull(dbo.Detail.BidItemId,0) when 0 then case when isnull(Detail.BidHeaderId,0) = 0 then dbo.Requisitions.BidHeaderId else Detail.BidheaderId end else dbo.Bids.BidHeaderId end end
      from dbo.Detail
...
--      left outer join BidHeaders on BidHeaders.BidHeaderId = case isnull(Detail.BidItemId,0) when 0 then isnull(Requisitions.BidHeaderId,0) else Bids.BidHeaderId end
     where Detail.ItemId is not null 
       ...
       and case isnull(Detail.BidItemId,0) when 0 then case when isnull(Detail.BidHeaderId,0) = 0 then isnull(Requisitions.BidHeaderId,0) else Detail.BidHeaderId end else Bids.BidHeaderId end = @pBidHeaderId
  ...
  Update @RetTable
    ...
    join (
          select rt1.ListId, rt1.DistrictId, rt1.BidHeaderId, rt1.VendorId, sum(ItemCount) ItemCount, sum(TotalAmount) TotalAmount
            from (
              select rt1.ListId, rt1.DistrictId, rt1.BidHeaderId, rt1.VendorId, count(rt1.ItemCode) ItemCount, /*case rt1.UseGrossPrices when 0 then sum(isnull(rt1.Quantity,0) * isnull(rt1.BidPrice,0)) else sum(isnull(rt1.Quantity,0) * isnull(rt1.GrossPrice,0)) - (sum(isnull(rt1.Quantity,0) * isnull(rt1.GrossPrice,0)) * isnull(rt1.DiscountRate,0) / 100) end*/ sum(rt1.ExtendedPrice) TotalAmount
                from @RetTable rt1
