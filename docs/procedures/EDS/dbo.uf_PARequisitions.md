# Function: table-valued: `dbo.uf_PARequisitions`

_Generated on 2026-05-04T14:49:07.404Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `uf_PARequisitions` |
| Kind | Function (table-valued) |
| sys.objects.type | `TF` (SQL_TABLE_VALUED_FUNCTION) |
| Created | 2006-06-27 12:53:53 |
| Modified | 2011-11-22 14:01:59 |
| Encrypted | no |
...
```sql
--select * from uf_PARequisitions(618919, null, 3055, null, null, null, 0/*33792673*/ /*50569889*/)


CREATE function [dbo].[uf_PARequisitions] (@pSessionId int, @pCategoryId int, @pBudgetId int, @pAccountId int, @pSchoolId int, @pUserId int, @pStatusId int)
...
insert @ReqList (SysId, RequisitionId, RequisitionNumber, Amount, Status, ApprovalLevel, 
                 AccountCode, DistrictName, SchoolName, CategoryName, Attention, CometId, CategoryId, 
  ...
  SELECT PendingApprovals.SysId, PendingApprovals.RequisitionId, Requisitions.RequisitionNumber, 
         ...
         isnull(StatusTable.Name,'On Hold') + isnull(case PendingApprovals.StatusId when 2 then ' by ' + left(NextApprover.Attention,20) else ' by ' + left(LastApprover.Attention,20) end,'') Status,
...
--         case isnull(st.StatusId,0) when 0 then 'All Statuses' else st.[Name] end
         case isnull(st.StatusId,0) when 0 then 'All Statuses' else isnull(st.Name,'') + case isnull(st.StatusId,0) when 0 then '' when 1 then '' when 2 then ' by ' + left(NextApprover.Attention,20) else ' by ' + left(LastApprover.Attention,20) end end
     from (
...
--         isnull(Approvals.StatusId,1 /*@OnHoldStatusId*/) StatusId,
         case isnull(Approvals.StatusId,0) when 0 then 1 when 3 then case isnull(Approvals.ApproverId,0) when 0 then 3 when Approvals.ApprovalById then 3 else 2 end else Approvals.StatusId end StatusId,
...
--    FROM PendingApprovals
    ...
    left outer JOIN StatusTable on StatusTable.StatusId = PendingApprovals.StatusId
    ...
    left outer join District on District.DistrictId = sb.DistrictId
    ...
    Left Outer Join StatusTable st on st.StatusId = ((@pStatusId & 0xFF000000) / 16777216)
