# Function: table-valued: `dbo.uf_SavingsLetterState`

_Generated on 2026-05-04T13:04:00.629Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `uf_SavingsLetterState` |
| Kind | Function (table-valued) |
| sys.objects.type | `TF` (SQL_TABLE_VALUED_FUNCTION) |
| Created | 2007-12-03 09:31:30 |
...
CREATE  function dbo.uf_SavingsLetterState (@pRSId int, @PassDate varchar(255)) 
...
insert @CatTable(DistrictId, CategoryId, OnSavings, DistrictName, CategoryName, 
                 ...
                 [BidItems], BidList, BidCost, State)
select DistrictId, CategoryId, isnull(OnSavingsReport,0), DistrictName, CategoryName, 
       ...
       sum(CatalogItem), sum(CatalogList), sum(CatalogCost), sum(BidItem), sum(BidList), sum(BidCost), State
  from (
...
--           case Category.CategoryId when 12 then round((Detail.BidPrice / .6),2) when 15 then (select top 1 CatalogPrice from CrossRefs join Catalog on Catalog.CatalogId = CrossRefs.CatalogId and Catalog.Name = 'EDS' and Catalog.Active = 1 where CrossRefs.ItemId = Detail.ItemId and CrossRefs.Active = 1 and CrossRefs.CatalogPrice < 9999 order by CatalogPrice desc) when 44 then round((Detail.BidPrice / .6),2) else isnull((select top 1 CatalogPrice from CrossRefs join Catalog on Catalog.CatalogId = CrossRefs.CatalogId and Catalog.Active = 1 join AwardsCatalogList on AwardsCatalogList.CatalogId = Catalog.CatalogId and AwardsCatalogList.AwardId = Awards.AwardId where CrossRefs.ItemId = Detail.ItemId and CrossRefs.Active = 1 and CrossRefs.CatalogPrice < 9999 order by Catalog.CatalogYear desc, CatalogPrice desc),isnull(Detail.CatalogPrice,Detail.BidPrice / .6)) end CatalogPrice, 
           case Category.CategoryId when 12 then round((Detail.BidPrice / .6),2) when 15 then (select top 1 CatalogPrice from CrossRefs join Catalog on Catalog.CatalogId = CrossRefs.CatalogId and Catalog.Name = 'EDS' and Catalog.Active = 1 where CrossRefs.ItemId = Detail.ItemId and CrossRefs.Active = 1 and CrossRefs.CatalogPrice < 9999 order by CatalogPrice desc) when 44 then round((Detail.BidPrice / .6),2) else isnull(Detail.CatalogPrice,Detail.BidPrice / .6) end CatalogPrice, 
           1 - (Detail.BidPrice / isnull(case Category.CategoryId when 12 then round((Detail.BidPrice / .6),2) when 15 then (select top 1 CatalogPrice from CrossRefs join Catalog on Catalog.CatalogId = CrossRefs.CatalogId and Catalog.Name = 'EDS' and Catalog.Active = 1 where CrossRefs.ItemId = Detail.ItemId and CrossRefs.Active = 1 and CrossRefs.CatalogPrice < 9999 order by CatalogPrice desc) when 44 then round((Detail.BidPrice / .6),2) else isnull((select top 1 CatalogPrice from CrossRefs join Catalog on Catalog.CatalogId = CrossRefs.CatalogId and Catalog.Active = 1 where CrossRefs.ItemId = Detail.ItemId and CrossRefs.Active = 1 and CrossRefs.CatalogPrice < 9999 order by CatalogPrice desc),isnull(Detail.CatalogPrice,Detail.BidPrice / .6)) end ,Detail.BidPrice / .6)) Discount, 
           sum(Detail.Quantity * Detail.BidPrice) BidExtended, 
           case Category.CategoryId when 12 then round((Detail.BidPrice / .6),2) when 15 then (select top 1 CatalogPrice from CrossRefs join Catalog on Catalog.CatalogId = CrossRefs.CatalogId and Catalog.Name = 'EDS' and Catalog.Active = 1 where CrossRefs.ItemId = Detail.ItemId and CrossRefs.Active = 1 and CrossRefs.CatalogPrice < 9999 order by CatalogPrice desc) when 44 then round((Detail.BidPrice / .6),2) else isnull((select top 1 CatalogPrice from CrossRefs join Catalog on Catalog.CatalogId = CrossRefs.CatalogId and Catalog.Active = 1 where CrossRefs.ItemId = Detail.ItemId and CrossRefs.Active = 1 and CrossRefs.CatalogPrice < 9999 order by CatalogPrice desc),isnull(Detail.CatalogPrice,Detail.BidPrice / .6)) end * sum(Detail.Quantity) CatalogExtended, 
           ...
           case isnull(Detail.BidItemId,0) 
             when 0 then 
               case Category.CategoryId 
                 when 15 then
                   (select top 1 CatalogPrice from CrossRefs join Catalog on Catalog.CatalogId = CrossRefs.CatalogId and Catalog.Name = 'EDS' and Catalog.Active = 1 where CrossRefs.ItemId = Detail.ItemId and CrossRefs.Active = 1 and CrossRefs.CatalogPrice < 9999 order by CatalogPrice desc)
                 else
...
--                   (select top 1 CatalogPrice from CrossRefs join Catalog on Catalog.CatalogId = CrossRefs.CatalogId and Catalog.Active = 1 join AwardCatalogList on AwardCatalogList.CatalogId = Catalog.CatalogId and AwardCatalogList.AwardId = Awards.AwardId where CrossRefs.ItemId = Detail.ItemId and CrossRefs.Active = 1 and CrossRefs.CatalogPrice < 9999 order by Catalog.CatalogYear desc, CatalogPrice desc)
             ...
             when 0 then 
               ...
               case Category.CategoryId 
                 when 15 then
                   ...
                   case isnull(Detail.CatalogPrice,0) when 0 then (select top 1 CatalogPrice from CrossRefs join Catalog on Catalog.CatalogId = CrossRefs.CatalogId and Catalog.Active = 1 where CrossRefs.ItemId = Detail.ItemId and CrossRefs.Active = 1 and CrossRefs.CatalogPrice < 9999 order by CatalogPrice desc) else Detail.CatalogPrice end
               end
     ...
     where case Category.CategoryId when 12 then round((Detail.BidPrice / .6),2) when 15 then (select top 1 CatalogPrice from CrossRefs join Catalog on Catalog.CatalogId = CrossRefs.CatalogId and Catalog.Name = 'EDS' and Catalog.Active = 1 where CrossRefs.ItemId = Detail.ItemId and CrossRefs.Active = 1 and CrossRefs.CatalogPrice < 9999 order by CatalogPrice desc) when 44 then round((Detail.BidPrice / .6),2) else isnull((select top 1 CatalogPrice from CrossRefs join Catalog on Catalog.CatalogId = CrossRefs.CatalogId and Catalog.Active = 1 where CrossRefs.ItemId = Detail.ItemId and CrossRefs.Active = 1 and CrossRefs.CatalogPrice < 9999 order by CatalogPrice desc),isnull(Detail.CatalogPrice,Detail.BidPrice)) end != 0
       ...
       and isnull(District.State,'') in ('NJ','NY','CT')
...
--       and District.DistrictId = 34 
     ...
     group by District.DistrictId, Category.CategoryId, Category.OnSavingsReport,
              ...
              Detail.CatalogPrice, Detail.Quantity, Awards.StateContractDiscount, Awards.AwardId, District.State ) ss
 group by DistrictId, CategoryId, OnSavingsReport, DistrictName, CategoryName, StateContractDiscount, ss.State
...
insert @CatTable(DistrictId, CategoryId, OnSavings, DistrictName, CategoryName, 
                 ...
                 OverallSavings, OverallDiscount, [CatalogItems], CatalogList, CatalogCost,
                 [BidItems], BidList, BidCost, State)
  select null, CategoryId, OnSavings, null, CategoryName,
         sum(UniqueItems), sum(TotalItems), sum(TotalBidCost), sum(TotalCatalogCost),
         sum(TotalStateContractCost), StateContractDiscount,
         sum(OverallSavings), sum(OverallDiscount), sum(CatalogItems), sum(CatalogList), sum(CatalogCost),
         sum(BidItems), sum(BidList), sum(BidCost), State
    from @CatTable ct
   group by CategoryId, OnSavings, CategoryName, StateContractDiscount, State

/*
declare CatCur cursor fast_forward read_only for
select ct.State, isnull(ct1.CategoryId,0) CategoryId, Category.Name CategoryName, ct.OnSavings
  from @CatTable ct
  join Category on Category.CategoryId = ct.CategoryId
  left outer join @CatTable ct1 on isnull(ct1.DistrictId,0) = isnull(ct.DistrictId,0)
                               and ct1.CategoryId = Category.CategoryId
 where ct.DistrictId is null
 group by ct.State, ct1.CategoryId, Category.Name, ct.OnSavings
 order by ct.State, Category.Name

open CatCur

fetch next from CatCur into @State, @CategoryId, @CategoryName, @OnSavings
