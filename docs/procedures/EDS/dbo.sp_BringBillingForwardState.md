# Procedure: `dbo.sp_BringBillingForwardState`

_Generated on 2026-05-04T13:43:18.720Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `sp_BringBillingForwardState` |
| Kind | Procedure |
| sys.objects.type | `P` (SQL_STORED_PROCEDURE) |
| Created | 2024-10-31 15:01:02 |
| Modified | 2024-11-11 11:57:58 |
| Encrypted | no |
...
|---|------|------|------|---------|
| 1 | `@State` | IN | char(2) |  |
| 2 | `@increasePercentage` | IN | decimal(11,5) |  |
...
```sql
--exec sp_BringBillingForwardState 'NY', 0--1.9
CREATE   procedure [dbo].[sp_BringBillingForwardState] @State char(2), @increasePercentage decimal(11,5) as
...
select 1, District.DistrictId, 
 ...
 where District.Active = 1
   and isnull(District.State,'') = @State
...
select case when (isnull(ChargeTypes.LM,0) = 1 /*or isnull(ChargeTypes.RTK,0) = 1*/) then dc.Amount + round(dc.Amount * @increasePercentage / 100,-1) else dc.Amount end, 
       bn.BudgetId, dc.ChargeTypeId, GETDATE(), District.DistrictId, 
       dc.Frequency, dc.FrequencyData, case when isnull(dc.Amount,0) = 0 then 0 else round(dc.Amount * @increasePercentage / 100,-1) / dc.Amount end
  from District
  join Budgets bc on bc.DistrictId = District.DistrictId
                 and cast(substring(bc.Name,1,4) as int) = case when month(getdate()) between 8 and 12 then year(getdate()) + 1 else year(getdate()) end
  join DistrictProposedCharges dc on dc.BudgetId = bc.BudgetId
...
select 1, dc.Amount, bn.BudgetId, isnull(DATEADD(year,1,dc.ChargeDate),CAST('05/01/' + cast(year(getdate()) as char(4)) as datetime)), dc.ChargeTypeId, GETDATE(), District.DistrictId, 
       dc.Frequency, dc.FrequencyData, dc.Repeats
  from District
  join Budgets bc on bc.DistrictId = District.DistrictId
                 and cast(substring(bc.Name,1,4) as int) = case when month(getdate()) between 8 and 12 then year(getdate()) + 1 else year(getdate()) end - 1
...
--                  and not (isnull(ChargeTypes.LM,0) = 1 or isnull(ChargeTypes.RTK,0) = 1)
  join Budgets bn on bn.DistrictId = District.DistrictId
                 and cast(substring(bn.Name,1,4) as int) = case when month(getdate()) between 8 and 12 then year(getdate()) + 1 else year(getdate()) end
...
select dc.Amount + round(dc.Amount * @increasePercentage / 100,-1), 
       bn.BudgetId, dc.ChargeTypeId, GETDATE(), District.DistrictId, 
       dc.Frequency, dc.FrequencyData, case when isnull(dc.Amount,0) = 0 then 0 else round(dc.Amount * @increasePercentage / 100,-1) / dc.Amount end
*/
insert DistrictCharges (Active, Amount, BudgetId, ChargeDate, ChargeTypeId, DateUpdated, DistrictId, Frequency, FrequencyData, Repeats)
select 1, case when (isnull(ChargeTypes.LM,0) = 1 /*or isnull(ChargeTypes.RTK,0) = 1*/) then dc.Amount + round(dc.Amount * @increasePercentage / 100,-1) else dc.Amount end, bn.BudgetId, null /*isnull(DATEADD(year,1,dc.ChargeDate),CAST('05/01/' + cast(year(getdate()) as char(4)) as datetime))*/, dc.ChargeTypeId, GETDATE(), District.DistrictId, 
       dc.Frequency, dc.FrequencyData, null /* dc.Repeats*/
  from District
  join Budgets bc on bc.DistrictId = District.DistrictId
                 and cast(substring(bc.Name,1,4) as int) = case when month(getdate()) between 8 and 12 then year(getdate()) + 1 else year(getdate()) end
  join DistrictProposedCharges dc on dc.BudgetId = bc.BudgetId
