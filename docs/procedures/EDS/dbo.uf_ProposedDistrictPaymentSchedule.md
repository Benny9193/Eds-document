# Function: table-valued: `dbo.uf_ProposedDistrictPaymentSchedule`

_Generated on 2026-05-04T13:07:57.691Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `uf_ProposedDistrictPaymentSchedule` |
| Kind | Function (table-valued) |
| sys.objects.type | `TF` (SQL_TABLE_VALUED_FUNCTION) |
| Created | 2017-10-27 11:45:00 |
| Modified | 2024-03-13 11:07:50 |
| Encrypted | no |
...
```sql
--select * from dbo.[uf_ProposedDistrictPaymentSchedule]('585D3A81-D116-4962-A1F7-804D54DCCCE2')
CREATE   function [dbo].[uf_ProposedDistrictPaymentSchedule] (@pId uniqueidentifier)
returns @PS table (
...
begin
  select @DistrictNameAndAddress = 
       ...
       @TotalLMCharges = (select sum(isnull(Amount,0)) from DistrictProposedCharges with (nolock) join Budgets on Budgets.BudgetId = DistrictProposedCharges.BudgetId and @StartDate = Budgets.StartDate join ChargeTypes on ChargeTypes.ChargeTypeId = DistrictProposedCharges.ChargeTypeId and not(isnull(ChargeTypes.RTK,0) = 1) where DistrictProposedCharges.DistrictId = District.DistrictId),
       @TotalRTKCharges = (select sum(isnull(Amount,0)) from DistrictProposedCharges with (nolock) join Budgets on Budgets.BudgetId = DistrictProposedCharges.BudgetId and @StartDate = Budgets.StartDate join ChargeTypes on ChargeTypes.ChargeTypeId = DistrictProposedCharges.ChargeTypeId and ChargeTypes.RTK = 1 and isnull(ChargeTypes.LM,0) = 0 where DistrictProposedCharges.DistrictId = District.DistrictId),
  ...
  declare CCur cursor read_only for
    select DistrictProposedCharges.ChargeTypeId, DistrictProposedCharges.Amount, DistrictProposedCharges.Frequency, DistrictProposedCharges.FrequencyData, 
	--       case DistrictProposedCharges.ChargeTypeId when 6 then case District.StateId when 2 then 'B2' else 'BY' end else case isnull(ChargeTypes.LM,0) when 1 then ChargeTypes.AccountingChargeCode else null end end, 
	--       case isnull(ChargeTypes.RTK,0) when 1 then ChargeTypes.AccountingChargeCode else null end, 
/*         case DistrictProposedCharges.ChargeTypeId 
             when 4 then ChargeTypes.AccountingChargeCode 
             when 5 then ChargeTypes.AccountingChargeCode 
             when 6 then 
               case District.StateId 
                 when 2 then 'B2' 
                 else 'BY' 
               end 
             when 7 then ChargeTypes.AccountingChargeCode 
             when 8 then ChargeTypes.AccountingChargeCode 
             else 
               case isnull(ChargeTypes.LM,0) 
                 when 1 then ChargeTypes.AccountingChargeCode 
                 else null 
               end 
           end, 
           case isnull(ChargeTypes.RTK,0) 
             when 1 then ChargeTypes.AccountingChargeCode 
             else null 
           end, 
*/
	   case 
	     when isnull(ChargeTypes.LM,0) = 0 and isnull(ChargeTypes.RTK,0) = 1 then null
		 else ChargeTypes.AccountingChargeCode
	   end,
	   case 
	     when isnull(ChargeTypes.LM,0) = 0 and isnull(ChargeTypes.RTK,0) = 1 then ChargeTypes.AccountingChargeCode
		 else null
	   end,
       Budgets.StartDate,
     ...
     where DistrictProposedCharges.BudgetId = @BudgetId
       and DistrictProposedCharges.Frequency > 0
      ...
      begin
        insert @PS(DistrictId, BudgetId, ProgramName, BudgetYear, OrderYear, DistrictName, DistrictNameAndAddress, TotalLMCharges, TotalRTKCharges, CDate, LMAmount, RTKAmount, RTK, AccountingDistrictCode, LMChargeCode, RTKChargeCode, Street1, City, State, Zipcode)
          values (@DistrictId, @BudgetId, @ProgramName, 
                  ...
                  case when @RTK = 1 and @LM = 0 then @WorkAmount 
                    else 0 
                  end, 
                  @RTK,
...
Update ps
   set DearMsg = 'Dear Board of Education:<br/><br/><p>Thank you for your support in making the ' + ProgramName + ' Cooperative Bid Maintenance Program the largest and most successful shared services program of its kind. This year participating districts received the lowest overall pricing for consumable school supplies in the state.</p>',
	   ...
	   [Status] = (select [Status] from DistrictContinuances where DistrictContinuances.Id = @pId),
	   SignedBy = (select [SignedBy] from DistrictContinuances where DistrictContinuances.Id = @pId),
	   SignedDate = (select [Received] from DistrictContinuances where DistrictContinuances.Id = @pId)
  from @PS ps
