# Function: table-valued: `dbo.uf_RequisitionList`

_Generated on 2026-05-04T14:49:07.413Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `uf_RequisitionList` |
| Kind | Function (table-valued) |
| sys.objects.type | `TF` (SQL_TABLE_VALUED_FUNCTION) |
| Created | 2009-01-06 15:54:14 |
| Modified | 2013-05-22 19:03:44 |
| Encrypted | no |
...
```sql
CREATE   function [dbo].[uf_RequisitionList] (@pSessionId int, @pCategoryId int, @pBudgetId int, @pAccountId int, @pSchoolId int, @pUserId int, @pStatusId int)
...
select @DistrictApprovalLevel = RequiredApprovalLevel,
	   @DistrictName = isnull(District.Name,'')
...
insert @@PendingApprovals (SessionId, SchoolId, UserId, RequisitionId, BudgetId, AccountId, CategoryId, Amount, ApprovalLevel, StatusId, LastApprovalId, RequisitionNumber,
  ...
  select @SessionId, Requisitions.SchoolId, Requisitions.UserId, 
	...
	substring(isnull((select top 1 StatusTable.Name + case when isnull(@pStatusId,0) < 100 then '' when isnull(@pStatusId,0) / 16777216 = 0 then '' else ' by ' + Users.Attention end from StatusTable left outer join Users on Users.UserId = @pStatusId & 16777215 where StatusTable.StatusId = case when isnull(@pStatusId,0) < 100 then isnull(@pStatusId,0) else isnull(@pStatusId,0) / 16777216 end),'All Statuses'),1,50)
...
begin
  delete @@pendingApprovals
    from @@PendingApprovals PendingApprovals
   where case when PendingApprovals.StatusId < 100 then PendingApprovals.StatusId else PendingApprovals.StatusId / 16777216 end != case when @pStatusId < 100 then @pStatusId else @pStatusId / 16777216 end
end
