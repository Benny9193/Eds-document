...
_Generated on 2026-05-04T13:04:24.062Z_
...
```sql
CREATE         procedure dbo.sp_BatchConvert @pSessionId int, @pBatchId int AS
...
/*
  declare CICur cursor fast_forward read_only for
    select BatchDetailId, ItemId, Qty, SourceId
      from BatchDetail 
     where BatchBookId = @BatchBookId
       and Active = 1
  ...
  fetch next from CICur into @BatchDetailId, @ItemId, @Qty, @SourceId

  while @@fetch_status = 0
  begin
    insert Detail (RequisitionId, ItemId, Quantity, BatchDetailId, SourceId)
      select top 1 @RequisitionId, li.ItemId, @Qty, @BatchDetailId, @SourceId
        from dbo.uf_LookupPriceByReq(@ItemId, @BidHeaderId) li
       order by case isnull(li.BidItemId,0) when 0 then 1 else 0 end, li.BidPrice, li.CrossRefId desc
...
/*
  insert Detail (RequisitionId, CatalogId, ItemId, AddendumItem, ItemCode, Quantity,
    ...
    select RequisitionId, CatalogId, ItemId, AddendumItem, ItemCode, Quantity,
      ...
      from #TempDetail
*/
  insert Detail (RequisitionId, ItemId, Quantity, LastYearsQuantity, BatchDetailId, SourceId)
    select RequisitionId, ItemId, Quantity, LastYearsQuantity, BatchDetailId, SourceId
      from #TempDetail
...
/*  -- Get Requisition Total
  select @Total = sum(ISNULL(Quantity,0) * ISNULL(BidPrice,0))
    from Detail
   where RequisitionId = @RequisitionId
*/
  -- Update Accounting
