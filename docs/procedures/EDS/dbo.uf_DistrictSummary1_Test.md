# Function: table-valued: `dbo.uf_DistrictSummary1_Test`

_Generated on 2026-05-04T13:04:00.541Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `uf_DistrictSummary1_Test` |
| Kind | Function (table-valued) |
| sys.objects.type | `TF` (SQL_TABLE_VALUED_FUNCTION) |
| Created | 2022-04-04 10:38:53 |
| Modified | 2022-04-04 10:38:53 |
| Encrypted | no |
...
```sql
create        function [dbo].[uf_DistrictSummary1_Test] (@pRSId int, @pCategoryId int)
...
as
begin
  insert @RetTable (ItemId, BidPrice, GrossPrice, Quantity, VendorId, VendorItemCode, Alternate, DistrictId, ListId, CategoryName, DistrictName, ItemCode, SortSeq, Description, UnitCode, VendorCode, UnitPrice, ExtendedPrice, DiscountRate, UseGrossPrices, CategoryId, PricePlanId, AwardId, BudgetId, BidHeaderId, ShippingCost)
    select dbo.Detail.ItemId,
           ...
           --case isnull(dbo.Detail.ItemMustBeBid,0) when 1 then case isnull(dbo.Detail.BidHeaderId,0) when 0 then null else dbo.Detail.BidHeaderId end else case isnull(dbo.Detail.BidHeaderId,0) when 0 then dbo.Requisitions.BidHeaderId else dbo.Detail.BidHeaderId end end
           BidHeaders.BidHeaderId,
		   isnull(Detail.ShippingCost,0)
      from dbo.Detail
      ...
      left outer join BidHeaders on BidHeaders.BidHeaderId = case isnull(Detail.BidItemId,0) when 0 then case isnull(Detail.BidHeaderId,0) when 0 then isnull(Requisitions.BidHeaderId,0) else Detail.BidHeaderId end else Bids.BidHeaderId end
     where detail.ItemId is not null and rsl.RSId = @pRSId and Requisitions.CategoryId = @pCategoryId
     group by rsl.RSId, dbo.Detail.ItemId, dbo.Detail.BidPrice, dbo.Detail.GrossPrice,
              ...
              isnull(Detail.UseGrossPrices,0), Requisitions.CategoryId, Detail.PricePlanId, 
              /*Detail.AwardId, */Requisitions.BudgetId, Detail.DiscountRate, 
              BidHeaders.BidHeaderId, isnull(Detail.ShippingCost,0)
--              case isnull(dbo.Detail.BidHeaderId,0) when 0 then dbo.Requisitions.BidHeaderId else dbo.Detail.BidHeaderId end
--           case isnull(dbo.Detail.ItemMustBeBid,0) when 1 then case isnull(dbo.Detail.BidHeaderId,0) when 0 then null else dbo.Detail.BidHeaderId end else case isnull(dbo.Detail.BidHeaderId,0) when 0 then dbo.Requisitions.BidHeaderId else dbo.Detail.BidHeaderId end end

  Update @RetTable
     Set VendorTotal = ss.TotalAmount + isnull(af.AddFreight,0),
    ...
    join (
          select rt1.ListId, rt1.DistrictId, rt1.BidHeaderId, rt1.VendorId, sum(ItemCount) ItemCount, sum(TotalAmount) TotalAmount
            from (
              select rt1.ListId, rt1.DistrictId, rt1.BidHeaderId, rt1.VendorId, count(rt1.ItemCode) ItemCount, case rt1.UseGrossPrices when 0 then sum(isnull(rt1.Quantity,0) * isnull(rt1.BidPrice,0)) else sum(isnull(rt1.Quantity,0) * isnull(rt1.GrossPrice,0)) - (sum(isnull(rt1.Quantity,0) * isnull(rt1.GrossPrice,0)) * isnull(rt1.DiscountRate,0) / 100) end + sum(rt1.ShippingCost) TotalAmount
                from @RetTable rt1
	...
	outer apply (Select sum(rsc.AdditionalHandlingAmount) AddFreight from vw_RequisitionShippingCosts rsc join ReportSessionLinks rsl on rsl.RSId = @pRSId and rsl.IntId = rsc.RequisitionId where rsc.VendorId = ss.VendorId and rsc.BidHeaderId = rt.BidHeaderId) af

  Update @RetTable
     Set CategoryTotal = ss.TotalAmount + isnull(af.AddFreight,0),
	...
	outer apply (Select sum(rsc.AdditionalHandlingAmount) AddFreight from vw_RequisitionShippingCosts rsc join ReportSessionLinks rsl on rsl.RSId = @pRSId and rsl.IntId = rsc.RequisitionId join Requisitions on Requisitions.RequisitionId = rsl.IntId and Requisitions.CategoryId = ss.CategoryId where rsc.VendorId = rt.VendorId and rsc.BidHeaderId = rt.BidHeaderId) af

  Update @RetTable
     Set DistrictTotal = ss.TotalAmount + isnull(af.AddFreight,0),
	...
	outer apply (Select sum(rsc.AdditionalHandlingAmount) AddFreight from vw_RequisitionShippingCosts rsc join ReportSessionLinks rsl on rsl.RSId = @pRSId and rsl.IntId = rsc.RequisitionId) af
