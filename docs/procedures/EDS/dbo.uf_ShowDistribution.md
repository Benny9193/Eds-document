# Function: table-valued: `dbo.uf_ShowDistribution`

_Generated on 2026-05-04T14:49:07.431Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `uf_ShowDistribution` |
| Kind | Function (table-valued) |
| sys.objects.type | `TF` (SQL_TABLE_VALUED_FUNCTION) |
| Created | 2002-06-11 13:27:45 |
...
```sql
CREATE function dbo.uf_ShowDistribution (@pRSId int)
...
begin
  ...
  select @QuantityNeeded = Detail.Quantity - (select sum(PODI.Quantity) 
                                                from @PODetailItems podi
                                               where podi.DetailId = Detail.DetailId)
    from Detail
  ...
  begin
    ...
    insert @PODetailItems (DetailId, BidItemId, ItemId, Quantity, ItemCode)
      Values (@DetailId, @BidItemId, @ItemId, @QuantityUsed, @ItemCode)
  end
