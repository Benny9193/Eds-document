# Function: table-valued: `dbo.uf_OrderBookTest`

_Generated on 2026-05-04T13:07:57.669Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `uf_OrderBookTest` |
| Kind | Function (table-valued) |
| sys.objects.type | `TF` (SQL_TABLE_VALUED_FUNCTION) |
| Created | 2008-10-07 15:00:55 |
...
CREATE function dbo.uf_OrderBookTest(@pOrderBookId int, @pDistrictId int, @pUserId int)
...
--        join dbo.DistrictPP on dbo.DistrictPP.DistrictId = dbo.Budgets.DistrictId
        join dbo.DistrictPP on dbo.DistrictPP.DistrictId = @pDistrictId
...
begin
  Update @OrderBook
   ...
   (select top 1 BidPrice
      from (
          select case isnull(bp.VendorId,0) when isnull(pc.VendorId,0) then case isnull(bp.CatalogId,0) when isnull(ob.CatalogId,0) then 1 else 0 end else -1 end IB, bp.*
            from vw_BudgetPrice bp with (nolock)
           where bp.ItemId = ob.ItemId
           ) CD 
     order by IB desc, SortKey
  ...
  begin
    ...
    begin
      ...
      begin
        Delete @OrderBook
          from @OrderBook ob
          join dbo.Items on Items.ItemId = ob.ItemId          where case isnull(Items.DistrictId,0) when 0 then @pDistrictId else Items.DistrictId end != @pDistrictId
