# Procedure: `dbo.usp_SearchItemsByReqHKDS`

_Generated on 2026-05-04T13:04:00.747Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `usp_SearchItemsByReqHKDS` |
| Kind | Procedure |
| sys.objects.type | `P` (SQL_STORED_PROCEDURE) |
| Created | 2018-08-07 23:53:11 |
| Modified | 2022-12-22 15:00:36 |
| Encrypted | no |
...
--exec dbo.usp_SearchItemsByReqHKDS 56371080, 0, -1, 'gloves', 505, 0
...
--declare @pRequisitionId int     = 54336163
--declare @pVendorId int		    = 0--8919--0--541 --2198
--declare @pHeadingId BigInt	    = 0---1
--declare @pFilter varchar(4096)  = 'Copy Paper'  --'TISSUE KLEENEX 100/BOX 28224'
--declare @pDistrictId int	    = 400
--declare @pDSHeadingId bigint    = 0-- 0--1521492941208
--declare @QueryType varchar(3)   = 'and'  
...
CREATE   procedure [dbo].[usp_SearchItemsByReqHKDS] 
  (
	  @pRequisitionId int
	, @pVendorId int
	, @pHeadingId bigint
...
declare @RetVal int = 1, @WorkCount int = 0
...
PricingConsolidatedId bigint null,
...
SDSAvail	int null
	...
	-- If KeywordId = 0 then grab everyting that has just HeadingId (ignore Keyword)
	-- If Keyword has value than also filter by Keyword
	if @pHeadingId != -1
	begin
...
*/
		...
		begin
		  ...
		  set @sql = @Sql + ' insert into @TempPricing (Id, Rank)
			...
			set @Sql = @sql + '  ''' + replace(@pFilter,'''','''''') + ''', ''bidType=[1]'
		  -- BidHeader filter
		  ...
		  -- HeadingId
		    if @HeadingId <> 0 and @KeyWordId = 0
				set @sql = @sql + '|headingId=[' + trim(cast(@HeadingId as char(20))) + ']'
		  -- KeywordId
		    if @KeyWordId <> 0
				set @sql = @sql + '|headingKeywordId=[' + trim(cast(@pHeadingId as char(20))) + ']'
		  -- CategoryId
		  ...
		  set @sql = @sql + ''', 600,''' + @QueryType
		  set @sql = @sql + ''');;select * from @TempPricing'
--		  set @sql = @sql + ''',1);;select * from @TempPricing'
...
--			 delete @TempPricing where Rank < ((@TopScore - @BottomScore) / 2)
          ...
          -------------------------------------------------
			 ...
			 set id = PricingConsolidatedId
				...
				, bidtype = pc.BidType
				, OrderCount = pc.OrderCounts
				, RowNumber = 0
			 from @TempPricing temp 
			 join SearchData.dbo.PricingConsolidated pc on pc.PricingConsolidatedId = temp.Id

		  select @WorkCount = count(*) from @TempPricing
		  if @QueryType = 'or' or (@QueryType = 'and' and @WorkCount != 0)
		  begin
--		    print 'Complete ' + @QueryType
			break
		  end
		  else
		  begin
--			print 'Changing Mode from ' + @QueryType + ' rows = ' + cast(@WorkCount as varchar)
			select @QueryType = 'or', @RetVal = 0
		  end
	...
	declare @SearchTermSplit table(
		...
		from @TempPricing tp
		join PricingConsolidated pc on pc.PricingConsolidatedId = tp.Id
			...
			insert @ItemTable
					(   PricingConsolidatedId,
						...
						,CatalogPage, PricePlanId, AwardId, VendorId, VendorItemCode, Alternate, BidItemId  --, ItemMustBeBid, ItemBidType)
   						, [Description], UnitId, UnitCode, SortSeq, ItemCode, BidType, OrderCount
					)
					select Id, tp.BidHeaderId, tp.ItemId, Rank, CatalogId, CrossRefId, BidPrice, CatalogPrice, DiscountRate
						 ...
						 , /*id.ItemDescription*/FullDescription as FullDescription, UnitId, UnitCode, SortSeq, ItemCode, BidType, OrderCount
			...
			if @HeadingId > 0
			  begin
				delete @ItemTable
				 ...
				 where not exists(select * from @HeadingsAndKeywords hk where hk.HeadingId = Items.HeadingId)
			  end
...
--select * from @ItemTable
		   ...
		   begin
			   insert @ItemTable(PricingConsolidatedId, BidHeaderId, [ItemId], Ranking, CatalogId, CrossRefId, BidPrice, CatalogPrice, DiscountRate
				  ...
				  select PricingAddendaId
				   ...
				   where ( isnull(items.DistrictId,0) in ( 0, @DistrictId))
   ...
   insert @ItemTable(PricingConsolidatedId,  BidHeaderId, [ItemId], Ranking, CatalogId, CrossRefId, BidPrice, CatalogPrice, DiscountRate
	...
	select pc.PricingConsolidatedId, pc.BidHeaderId, pc.ItemId, 0, CatalogId, CrossRefId, BidPrice, CatalogPrice, DiscountRate
		 ...
		 , /*id.ItemDescription*/ FullDescription as FullDescription, UnitId, UnitCode, SortSeq, ItemCode, BidType, pc.OrderCounts
...
--		SearchTermNum = /*Num terms */ right('00' + cast(0 as varchar(2)),2) + case isnull(BidItemId,0) when 0 then '1' else '0' end + /*Qty sold  + n*/ right('000000' + cast(999999 - isnull(it.Quantity,0) as varchar(6)),6),
		SearchTermNum = CASE ISNULL(Detail.Quantity,0) WHEN 0 THEN '1' ELSE '0' END, --+ cast(100000-cast(isnull(Ranking,0) as int) as varchar(15)) + case isnull(BidItemId,0) when 0 then '1' else '0' end,
-- **original line **		SearchTermNum = /*Num terms */ right('00' + cast(99 - isnull(NumberTerms,0) as varchar(2)),2) + case isnull(BidItemId,0) when 0 then '1' else '0' end + /*Qty sold  + n*/ right('000000' + cast(999999 - isnull(it.Quantity,0) as varchar(6)),6),
		ImageURL = xr.ImageURL,
		FullDescription = coalesce(xr.FullDescription,Description,''),
		SDSAvail = xr.SDSAvail
    from @ItemTable it
	...
	outer apply (select ImageURL, FullDescription, case when trim(coalesce(MSDSRef,'')) like 'http%' then 1 else 0 end SDSAvail from CrossRefs where CrossRefs.CrossRefId = it.CrossRefId) xr
--	outer apply (select OrderCount from PricingConsolidatedOrderCounts oc where oc.BidHeaderId = it.BidHeaderId and oc.ItemId = it.ItemId) OrderCounts
	  ...
	  --select PricingConsolidatedId, Description,  VendorName , BidPrice from @ItemTable where BidPrice = 0
  ...
  --
 					SELECT  
							...
							,u.UnitCode
							,u.LastYearsQuantity
							,Quantity = '<input type="text" id="SearchItemstable_Quantity_'+CAST(u.ItemID AS varchar)+'" value="'+CASE ISNULL(u.Quantity,0) WHEN 0 THEN '' ELSE CAST(u.Quantity AS varchar) END+'" pos="' + cast(row_number() over (order by case when u.Quantity is null or u.Quantity = 0 then 0 else 1 end desc, u.Ranking desc, case when u.BidItemId is null or u.BidItemId = 0 then 0 else 1 end desc, OrderCount desc, u.BidPrice) as varchar(20)) + '" tabindex class="qty" onKeydown="SetEnterFunction(this,event);" onFocus="SetFocusFunction(this,event);" onChange="SetBlurFunction(this,event);">' 
							, u.Ranking
							,u.BidPrice
							...
							, BidItemId
							, SearchTermNum = CASE ISNULL(u.Quantity,0) WHEN 0 THEN '1' ELSE '0' END 
--							, SortKey =  SearchTermNum + cast(100000-(isnull(Ranking,0)) as varchar(15)) + case when BidItemId is null or BidItemId = 0 then '1' else '0' end + cast(1000000 - isnull(OrderCount,0) AS varchar(15))
							, SortKey =  SearchTermNum + right(replicate('0',10) + cast(1000000 - isnull(u.OrderCount,0) as varchar(10)),10)
							...
							, SearchTermNum + right('0000000000' + cast(cast(u.BidPrice * 100 as int) as varchar(20)),10) Sort_BidPrice
							, SearchTermNum + u.VendorItemCode Sort_VendorItemCode
							, SearchTermNum + u.SortSeq Sort_EDSSortKey
							, u.Ranking
							, u.OrderCount
--							, SearchTermNum + cast(100000-(isnull(Ranking,0)) as varchar(15)) defaultSort
							, SearchTermNum + right(replicate('0',10) + cast(1000000 - isnull(u.OrderCount,0) as varchar(10)),10) defaultSort
							, isnull(u.ImageURL,'') ImageURL
							, '<img class="searchThumbnail" src="' + isnull(u.ImageURL,'') + '" />' Thumbnail
							, u.FullDescription
							, u.Quantity RawQuantity
							, isnull(u.BidItemId,0) BidItemId
							, isnull(SDSAvail,0) SDSAvail
							, row_number() over (order by case when u.Quantity is null or u.Quantity = 0 then 0 else 1 end desc, u.Ranking desc, case when u.BidItemId is null or u.BidItemId = 0 then 0 else 1 end desc, OrderCount desc, u.BidPrice) rowNumber
							, case when isnull(BidItemId,0) = 0 then '1' else '0' end + right(replicate('0',18) + cast(row_number() over (order by case when u.Quantity is null or u.Quantity = 0 then 0 else 1 end desc, u.Ranking desc, case when u.BidItemId is null or u.BidItemId = 0 then 0 else 1 end desc, OrderCount desc, u.BidPrice) as varchar(18)),18) Sort_BidFirst
					FROM	@ItemTable u
					order by case when u.Quantity is null or u.Quantity = 0 then 0 else 1 end desc, u.Ranking desc, case when u.BidItemId is null or u.BidItemId = 0 then 0 else 1 end desc, OrderCount desc, u.BidPrice
--					order by u.Ranking desc, case when BidItemId is null or BidItemId = 0 then 0 else 1 end desc, OrderCount desc
/*					order by SearchTermNum + right(replicate('0',18) + cast(1000000000000000 - 
					                                                   cast(
																	   cast(u.Ranking as float) * 
																	   cast(isnull(u.OrderCount,0) as float) as bigint) as varchar(30)),18)
*/
...
--*/
	...
	-- Return weather or not we swapped modes 0 = swapped modes 1 = original mode
	return @RetVal
