# Procedure: `dbo.sp_CopyMSRPVers4Bid`

_Generated on 2026-05-04T13:04:00.337Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `sp_CopyMSRPVers4Bid` |
| Kind | Procedure |
| sys.objects.type | `P` (SQL_STORED_PROCEDURE) |
| Created | 2022-10-21 16:10:30 |
| Modified | 2022-10-21 16:10:30 |
| Encrypted | no |
...
-- =============================================
-- Author:		Kevin / David
-- Create date: 1-17-2014, updated 10/21/2022 
-- Description:	Copy MSRP Bid (Version 4)
-- =============================================
CREATE procedure [dbo].[sp_CopyMSRPVers4Bid] @pOldBidHeaderId  varchar(50), @pNewBidHeaderId  varchar(50), @ExcludeManufWithBids  tinyint = 1, @IncludeWriteInManuf  tinyint = 1
AS
BEGIN
...
BEGIN
  ...
  INSERT INTO BidRequestOptions
    (BidHeaderId, ManufacturerId, ManufacturerProductLineId, OptionId, BidRequestManufacturerId, BidRequestProductLineId, [Name], [Weight])
  SELECT @CopyToBidHeaderId, VBRM.ManufacturerId, VBRM.ManufacturerProductLineId, OABRO.MSRPOptionId, BRM.BidRequestManufacturerId, BRPL.BidRequestProductLineId, OABRO.OptionName, OABRO.Weight
  FROM vw_BidMSRPRankedManufacturerProductLinesOrdered VBRM
  JOIN BidRequestManufacturer BRM ON BRM.BidHeaderId = @CopyToBidHeaderId 
                                 and BRM.ManufacturerId = VBRM.ManufacturerId 
  JOIN BidRequestProductLines BRPL ON BRPL.BidRequestManufacturerId = BRM.BidRequestManufacturerId
                                  ...
                                  and BRPL.Active = 1
  -- This code gets the Default BidRequestOption(s) for Writeins 
  OUTER APPLY (select BidRequestOptions.OptionId MSRPOptionId, BidRequestOptions.Name OptionName, BidRequestOptions.Weight  
  ...
  LEFT OUTER JOIN BidRequestOptions BRO on BRO.BidHeaderId = BRM.BidHeaderId
                                       and BRO.BidRequestManufacturerId = BRM.BidRequestManufacturerId
                                                                 ...
                                                                 and BRO.OptionId = OABRO.MSRPOptionId
  WHERE VBRM.BidHeaderId = @CopyFromBidHeaderId 
    AND VBRM.AllActive = 1
    AND VBRM.WriteInFlag = 1 
    -- only populate records where no option set yet
    AND BRO.BidRequestOptionId is null 
  group by VBRM.ManufacturerId, VBRM.ManufacturerProductLineId, OABRO.MSRPOptionId, BRM.BidRequestManufacturerId, BRPL.BidRequestProductLineId, OABRO.OptionName, OABRO.Weight
  ...
  -- This code gets the Default BidRequestPriceRanges for WriteIns
  outer apply (select BidRequestPriceRanges.RangeBase, BidRequestPriceRanges.RangeWeight
              ...
              join ManufacturerProductLines on ManufacturerProductLines.ManufacturerId = Manufacturers.ManufacturerId
                                           and ManufacturerProductLines.ManufacturerProductLineId = BidRequestOptions.ManufacturerProductLineId
              where BidRequestOptions.BidHeaderId = @CopyFromBidHeaderId
                and BidRequestOptions.OptionId = BRO.OptionId 
			  ) OABRPR 
  ...
  WHERE VBRM.BidHeaderId = @CopyFromBidHeaderId 
    AND VBRM.AllActive = 1
    AND VBRM.WriteInFlag = 1 
    -- only populate records where no PRICE RANGE set yet  
