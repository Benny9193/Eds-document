...
_Generated on 2026-05-04T14:49:07.387Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `uf_LookupItemCodeByReqVendor` |
| Kind | Function (table-valued) |
| sys.objects.type | `TF` (SQL_TABLE_VALUED_FUNCTION) |
| Created | 2012-04-11 11:26:20 |
| Modified | 2025-09-18 15:12:46 |
| Encrypted | no |
...
CREATE function [dbo].[uf_LookupItemCodeByReqVendor] (@pRequisitionId int, @pItemCode varchar(255), @pVendorId int)
returns @ItemTable table (
  ...
  DigitallyDelivered TINYINT,
  MinimumOrderQuantity INT,
  SDSURL varchar(255) null
)
...
declare @catlist table (CatalogId int, PackedCode varchar(50))
  ...
  select @CategoryId = isnull(Requisitions.CategoryId,0),
		 ...
		 @SDSEnabled = coalesce(District.EnableRTKOnline,0)
...
--  insert DebugMsgs (Msg) values ((select Detail.DetailId, Detail.CatalogId, Detail.BidItemId, Detail.BidPrice, Detail.ItemMustBeBid from inserted join Detail on Detail.DetailId = inserted.DetailId for xml auto))
  ...
  Update it
     set CatalogId = BestBid.CatalogId,
        ...
        ,PerishableItem = BestBid.PerishableItem
        ,PrescriptionRequired = BestBid.PrescriptionRequired
        ,DigitallyDelivered = BestBid.DigitallyDelivered
        ,MinimumOrderQuantity = BestBid.MinimumOrderQuantity
    from @ItemTable it
	...
	outer apply (select top 1 CrossRefs.CatalogId, 
						...
						BidItems.CrossRefId,
						BidHeaders.BidType,
			  		    SDSAvail = case when @SDSEnabled = 1 and (isnull(trim(CrossRefs.MSDSRef),'') like 'http%' or isnull(trim(BidResults.SDS_URL),'') like 'http%') then 1 else 0 end,
						coalesce(BidResults.PerishableItem,CrossRefs.PerishableItem,0) PerishableItem,
						coalesce(BidResults.PrescriptionRequired,CrossRefs.PrescriptionRequired,0) PrescriptionRequired,
						coalesce(BidResults.DigitallyDelivered,CrossRefs.DigitallyDelivered,0) DigitallyDelivered,
						coalesce(BidResults.MinimumOrderQuantity,CrossRefs.MinimumOrderQuantity,0) MinimumOrderQuantity,
						SDSURL = case 
								   when @SDSEnabled = 1 then
								     case 
						               when isnull(trim(BidResults.SDS_URL),'') like 'http%' then trim(BidResults.SDS_URL)
								       when isnull(trim(CrossRefs.MSDSRef),'') like 'http%' then trim(CrossRefs.MSDSRef)
								       else null
								     end
								   else null
								 end
...
--  insert DebugMsgs (Msg) values ((select Detail.DetailId, Detail.CatalogId, Detail.BidItemId, Detail.BidPrice, Detail.ItemMustBeBid from inserted join Detail on Detail.DetailId = inserted.DetailId for xml auto))
  Update it
     set CatalogId = CrossRefs.CatalogId,
         ...
         AdditionalShipping = isnull(CrossRefs.AdditionalShipping,0),
  		  BidType = BidHeaders.BidType,
  		  SDSAvail = case when @SDSEnabled = 1 and isnull(trim(CrossRefs.MSDSRef),'') like 'http%' then 1 else 0 end,
        ...
        DigitallyDelivered = CrossRefs.DigitallyDelivered,
        MinimumOrderQuantity = CrossRefs.MinimumOrderQuantity,
		SDSURL = case when @SDSEnabled = 1 then case when isnull(trim(CrossRefs.MSDSRef),'') like 'http%' then trim(CrossRefs.MSDSRef) else null end else null end
    from @Itemtable it
    join Category on Category.CategoryId = @CategoryId
                 and Category.Type in (1,2,4)
  ...
  Update it
	...
	outer apply (Select top 1 xr.CatalogId, xr.Page, xr.MSDSRef, xr.CrossRefId, xr.PerishableItem, xr.PrescriptionRequired, xr.DigitallyDelivered, xr.MinimumOrderQuantity
	               from CrossRefs xr
				   join Catalog cat on Cat.CatalogId = xr.CatalogId
								   and Cat.Active = 1
				  ...
				  where xr.ItemId = it.ItemId
				    and xr.Active = 1
				  order by case isnull(xr.GrossPrice,0) when 0 then 0 else 1 end desc, xr.CatalogYear desc, isnull(xr.GrossPrice,0), xr.CrossRefId desc) CrossRefs
/*				   
...
--  insert DebugMsgs (Msg) values ((select Detail.DetailId, Detail.CatalogId, Detail.BidItemId, Detail.BidPrice, Detail.ItemMustBeBid from inserted join Detail on Detail.DetailId = inserted.DetailId for xml auto))
  Update it
    ...
    join DistrictCategories on DistrictCategories.DistrictId = Budgets.DistrictId
                           ...
                           and DistrictCategories.AllowAddenda = 1
    join Items on Items.ItemId = it.ItemId
	outer apply (Select top 1 xr.CatalogId, xr.Page, xr.CatalogPrice, xr.GrossPrice, xr.CrossRefId
	               from CrossRefs xr
				   join Catalog cat on Cat.CatalogId = xr.CatalogId
								   and Cat.Active = 1
								   and Cat.CategoryId = @CategoryId
...
--    join vw_ItemDescription dd on dd.ItemId = it.ItemId
  ...
  Update it
     set ItemCount = (select count(*) from @ItemTable),
         ...
         VendorName = coalesce(vc.VendorName,Vendors.DisplayAs,Vendors.Name,''),
	...
	left outer join VendorCategory vc on vc.VendorId = Vendors.VendorId
	                                 and vc.CategoryId = it.CategoryId
