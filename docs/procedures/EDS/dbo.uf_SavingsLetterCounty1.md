# Function: table-valued: `dbo.uf_SavingsLetterCounty1`

_Generated on 2026-05-04T14:49:07.418Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `uf_SavingsLetterCounty1` |
| Kind | Function (table-valued) |
| sys.objects.type | `TF` (SQL_TABLE_VALUED_FUNCTION) |
| Created | 2008-11-13 00:18:14 |
...
CREATE  function dbo.uf_SavingsLetterCounty1 (@pRSId int, @PassDate varchar(255)) 
...
select DistrictId, CategoryId, isnull(OnSavingsReport,0), DistrictName, CategoryName, 
  ...
  from (
    select District.DistrictId, Category.CategoryId, isnull(Category.OnSavingsReport,0) OnSavingsReport,
           ...
           Category.Name CategoryName, isnull(District.County,'') as CountyName, isnull(District.STate,'') as StateName, 
           Detail.ItemCode, sum(Detail.Quantity) Quantity, Detail.BidPrice, 
             ...
             when 0 then 
               ...
               case Category.CategoryId 
                 when 15 then
                   ...
                   (select top 1 CatalogPrice from CrossRefs join Catalog on Catalog.CatalogId = CrossRefs.CatalogId and Catalog.Active = 1 where CrossRefs.ItemId = Detail.ItemId and CrossRefs.Active = 1 and CrossRefs.CatalogPrice < 9999 order by CatalogPrice desc) 
               end
...
--       and District.DistrictId = 34 
     ...
     group by District.DistrictId, Category.CategoryId, Category.OnSavingsReport,
              ...
              District.City, District.State, District.Zipcode, Category.Name, District.County,
 ...
 group by DistrictId, CategoryId, OnSavingsReport, DistrictName, CategoryName, CountyName, StateName, StateContractDiscount
...
begin
  Update @CatTable
     set CategoriesIncluded = isnull(CategoriesIncluded,'') + case @OnSavings when 1 then case rtrim(isnull(CategoriesIncluded,'')) when '' then '' else ', ' end + @CategoryName else '' end,
         CategoriesExcluded = isnull(CategoriesExcluded,'') + case @OnSavings when 0 then case rtrim(isnull(CategoriesExcluded,'')) when '' then '' else ', ' end + @CategoryName else '' end
