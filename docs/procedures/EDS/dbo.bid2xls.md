# Procedure: `dbo.bid2xls`

_Generated on 2026-05-04T13:07:57.291Z_
...
| Created | 2008-08-26 14:13:13 |
| Modified | 2022-07-22 13:38:02 |
| Encrypted | no |
...
```sql
--exec bid2xls 9708
CREATE   procedure [dbo].[bid2xls] @pBidHeaderId int
as
...
--       and case bh.PricePlanId when 36 then BidHeaders.PricePlanId else bh.PricePlanId end = BidHeaders.PricePlanId
	   ...
	   and (   getdate() between bh.EffectiveFrom and bh.EffectiveUntil
            or (    bh.EffectiveFrom between dateadd(month,-14,isnull(min(BidHeaders.EffectiveFrom),cast('11/01/' + cast(year(getdate()) as char(4)) as datetime))) and dateadd(month,-10,isnull(max(BidHeaders.EffectiveFrom),cast('11/01/' + cast(year(getdate()) as char(4)) as datetime)))
...
--       and case bh.PricePlanId when 36 then BidHeaders.PricePlanId else bh.PricePlanId end = BidHeaders.PricePlanId
       and bh.Active = 1
	   and (getdate() between bh.EffectiveFrom and bh.EffectiveUntil
            or (    bh.EffectiveFrom between dateadd(month,-14,isnull(BidHeaders.EffectiveFrom,cast('11/01/' + cast(year(getdate()) as char(4)) as datetime))) and dateadd(month,-11,isnull(BidHeaders.EffectiveFrom,cast('11/01/' + cast(year(getdate()) as char(4)) as datetime)))
...
--       and case bh.PricePlanId when 36 then BidHeaders.PricePlanId else bh.PricePlanId end = BidHeaders.PricePlanId
       and bh.Active = 1
	   and (getdate() between bh.EffectiveFrom and bh.EffectiveUntil
            or (    bh.EffectiveFrom between dateadd(month,-14,isnull(BidHeaders.EffectiveFrom,cast('11/01/' + cast(year(getdate()) as char(4)) as datetime))) and dateadd(month,-10,isnull(BidHeaders.EffectiveFrom,cast('11/01/' + cast(year(getdate()) as char(4)) as datetime)))
...
*/
  select @Part1 = @Part1 +
        ...
        case when @IncludeManufacturer = 1 then ' isnull(xr' + cast(@VendorId as varchar(16)) + '.Manufacturor,'''') as [' + @VendorName + ' Manufacturer],' else '' end +
        ' isnull(xr' + cast(@VendorId as varchar(16)) + '.ManufacturorPartNumber,'''') as [' + @VendorName + ' Manufacturer Part Number],' +
		' isnull(xr' + cast(@VendorId as varchar(16)) + '.Page,'''') as [' + @VendorName + ' Catalog Page],' +
		' isnull(xr' + cast(@VendorId as varchar(16)) + '.Name,'''') as [' + @VendorName + ' Catalog Name],' +
		' isnull(br' + cast(@VendorId as varchar(16)) + '.VendorItemCode,'''') as [' + @VendorName + ' Last Item Bid],' +
		' isnull(cast(br' + CAST(@VendorId as varchar(16)) + '.BidHeaderId as varchar(16)),'''') as [' + @VendorName + ' Last Bid],'
  ...
  select @Part2 = @Part2 +
		...
		' where BidResults.ItemId = Items.ItemId' +
		' and BidResults.Active = 1' +
		' and BidResults.ItemBidType in (''S'',''N'',''C'',''A'')' +
