...
_Generated on 2026-05-04T13:43:18.913Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `sp_ShowTextbookSavings` |
| Kind | Procedure |
| sys.objects.type | `P` (SQL_STORED_PROCEDURE) |
| Created | 2002-06-11 13:45:33 |
...
```sql
CREATE procedure sp_ShowTextbookSavings @pRSId int AS
...
begin
  ...
  begin
    ...
    select @VendorCode = Vendors.Code,
           @VendorName = Vendors.Name
      from Vendors
      join Bids on Bids.VendorId = Vendors.VendorId
      join BidItems on BidItems.BidId = Bids.BidId
     where BidItemId = @BidItemId

    -- Distribute Appropriate Quantity for Item
    insert PODetaillItems (DetailId, BidItemId, ItemId, Quantity, VendorCode, VendorName)
      Values (@DetailId, @BidItemId, @ItemId, @QuantityUsed, @VendorCode, @VendorName)
  end
...
select Vendors.Code VendorCode, Vendors.Name VendorName, count(*) LineItems,
sum(Quantity) TotalBooks, (sum(Quantity) * GrossPrice) GrossCost, 
(sum(Quantity) * BidPrice) BidCost
 from #PODetailItems
 join Vendors on Vendors.VendorId = #PODetailItems.VendorId
 group by Vendors.Code, Vendors.Name, #PODetailItems.GrossPrice, #PODetailItems.BidPrice
