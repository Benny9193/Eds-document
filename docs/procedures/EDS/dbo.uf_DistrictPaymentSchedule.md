# Function: table-valued: `dbo.uf_DistrictPaymentSchedule`

_Generated on 2026-05-04T13:07:57.605Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `uf_DistrictPaymentSchedule` |
| Kind | Function (table-valued) |
| sys.objects.type | `TF` (SQL_TABLE_VALUED_FUNCTION) |
| Created | 2007-04-04 10:21:47 |
| Modified | 2023-01-12 12:09:22 |
| Encrypted | no |
...
| Caller | Type |
|--------|------|
| `dbo.vw_DistrictPaymentSchedule` | VIEW |

## Definition

```sql
CREATE function [dbo].[uf_DistrictPaymentSchedule] ()
returns @PS table (
...
Zipcode			varchar(10),
PODiskAmount	money,
PODiskCode		varchar(50),
GenericPOAmount money,
GenericPOCode	varchar(50),
EPOAmount		money,
EPOCode			varchar(50)
)
...
--                  and Budgets.EndDate >= dateadd(year,1,@StartDate)
  ...
  begin
    ...
    begin
      ...
      begin
        insert @PS(DistrictId, BudgetId, ProgramName, BudgetYear, OrderYear, DistrictName, DistrictNameAndAddress, TotalLMCharges, TotalRTKCharges, CDate, LMAmount, RTKAmount, RTK, AccountingDistrictCode, LMChargeCode, RTKChargeCode, Street1, City, State, Zipcode, PODiskAmount, PODiskCode, GenericPOAmount, GenericPOCode, EPOAmount, EPOCode)
          values (@DistrictId, @BudgetId, @ProgramName, (Select cast(Year(StartDate)-1 as char(4)) + '-' + cast(Year(StartDate) as char(4)) from Budgets with (nolock) where Budgets.BudgetId = @BudgetId), (Select cast(Year(StartDate) as char(4)) + '-' + cast(Year(StartDate)+1 as char(4)) from Budgets with (nolock) where Budgets.BudgetId = @BudgetId), @DistrictName, @DistrictNameAndAddress, 
                  ...
                  @Zipcode,
                  case @ChargeId
                    when 4 then @WorkAmount
                    else 0
                  end,
                  case @ChargeId
                    when 4 then @LMChargeCode
                    else null
                  end,
                  case @ChargeId
                    when 7 then @WorkAmount
                    else 0
                  end,
                  case @ChargeId
                    when 7 then @LMChargeCode
                    else null
                  end,
                  case @ChargeId
                    when 8 then @WorkAmount
                    else 0
                  end,
                  case @ChargeId
                    when 8 then @LMChargeCode
                    else null
                  end)
      end
      else
      begin
        Update ps
           set LMAmount = isnull(LMAmount,0) + case @LM
               ...
               RTKAmount = isnull(RTKAmount,0) + case @RTK
                                                   when 1 then @WorkAmount
                                                   else 0
                                                 end,
               RTK = @RTK,
               ...
               RTKChargeCode = coalesce(case @RTK
                                          when 1 then @RTKChargeCode
                                          else null
                                        end,RTKChargeCode),
               PODiskAmount = coalesce(PODiskAmount,0) + case @ChargeId
                                                           when 4 then @WorkAmount
                                                           else 0
                                                         end,
               GenericPOAmount = coalesce(GenericPOAmount,0) + case @ChargeId
                                                                 when 7 then @WorkAmount
                                                                 else 0
                                                               end,
               ePOAmount = coalesce(GenericPOAmount,0) + case @ChargeId
                                                                 when 8 then @WorkAmount
                                                                 else 0
                                                               end,
               PODiskCode = coalesce(case @ChargeId
                                       when 4 then @LMChargeCode
                                       else null
                                     end,PODiskCode),
               GenericPOCode = coalesce(case @ChargeId
                                          when 7 then @LMChargeCode
                                          else null
                                        end,GenericPOCode),
               ePOCode = coalesce(case @ChargeId
                                          when 8 then @LMChargeCode
                                          else null
                                        end,ePOCode)
          from @PS ps
...
Update ps
   set DearMsg = '{\rtf1\ansi\deff0{{\fonttbl{\f0 Times New Roman;}}\fs24 Dear Board of Education:\par\par Thank you for your support in making the ' + ProgramName + ' Cooperative Bid Maintenance Program the largest and most successful shared services program of its kind. Participating districts received the lowest overall pricing for consumable school supplies in the state.  We have always regarded our member districts as partners.  With that in mind, and considering economic pressures facing many school districts, Educational Data is not increasing our fee for a second year in a row.\pard}}',
