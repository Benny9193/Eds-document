# Function: table-valued: `dbo.uf_OrderBookTest1`

_Generated on 2026-05-04T13:07:57.671Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `uf_OrderBookTest1` |
| Kind | Function (table-valued) |
| sys.objects.type | `TF` (SQL_TABLE_VALUED_FUNCTION) |
| Created | 2010-11-24 14:33:56 |
| Modified | 2010-11-24 16:05:18 |
| Encrypted | no |
...
CREATE function [dbo].[uf_OrderBookTest1](@pOrderBookId int, @pDistrictId int, @pUserId int)
...
*/
  ...
  from @OrderBook ob
  join vw_ItemPricingTest ip on ip.ItemId = ob.ItemId
...
--           and MustKeep != 1 -- added back 11/24/10
           and @PrintMode != 5
        end
      end
    end
    ...
    begin
      ...
      begin
        ...
        Delete @OrderBook
         where isnull(case isnumeric(CatalogPage) when 1 then convert(int,CatalogPage) else 0 end,0) = case @KeepZeroPages when 1 then -1 else 0 end
--           and isnull(MustKeep,0) != 1 -- Added Back 11/24/10
      end
    end
  ...
  -- This should remove the Furniture Items ... per John 9/11/2003
    ...
    begin
      delete @OrderBook
        ...
        join dbo.CrossRefs on CrossRefs.CrossRefId = ob.CrossRefId
