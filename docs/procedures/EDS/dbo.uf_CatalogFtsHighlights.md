# Function: table-valued: `dbo.uf_CatalogFtsHighlights`

_Generated on 2026-05-04T14:49:07.355Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `uf_CatalogFtsHighlights` |
| Kind | Function (table-valued) |
| sys.objects.type | `TF` (SQL_TABLE_VALUED_FUNCTION) |
| Created | 2012-06-06 23:38:36 |
| Modified | 2017-12-19 12:56:25 |
| Encrypted | no |
...
|---|------|------|------|---------|
| 1 | `@pCatalogId` | IN | int |  |
| 2 | `@pSearchParams` | IN | varchar(4000) |  |
...
--select * from dbo.uf_CatalogFtsHighlights(2537, '9-1369009-030')

CREATE function [dbo].[uf_CatalogFtsHighlights](@pCatalogId int, @pSearchParams varchar(4000))
returns @Hits table (
Page int,
Offset int,
Length int)
...
select PageNbr - Catalog.Page1 + 1, TextData
  from CatalogText with (nolock)
  join Catalog on Catalog.BaseCatalogId = CatalogText.CatalogId
              and Catalog.CatalogId = @pCatalogId
 where master.dbo.ufn_RegExIsMatch(CatalogText.TextData,@RegExMatchKey,1) != 0
/*select PageNbr - Catalog.Page1 + 1, TextData 
  from containstable(CatalogText, TextData, @PatMatchKey) ct
  join CatalogText on CatalogText.CatalogTextId = ct.[Key]
                  and CatalogText.CatalogId = @pCatalogId
  join Catalog on Catalog.CatalogId = CatalogText.CatalogId*/
    
open sc

fetch next from sc into @Page, @Text

while @@FETCH_STATUS = 0
begin
  insert @Hits (Page, Offset, Length)
  select @Page, MatchIndex, MatchLength
