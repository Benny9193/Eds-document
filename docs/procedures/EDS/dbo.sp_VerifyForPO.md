# Procedure: `dbo.sp_VerifyForPO`

_Generated on 2026-05-04T13:04:00.488Z_
...
| Created | 2015-03-23 21:31:50 |
| Modified | 2015-03-23 21:31:50 |
| Encrypted | no |
...
```sql
create procedure [dbo].[sp_VerifyForPO] @pRSId int
as
declare @we table(RequisitionId int, ErrorMsg varchar(255))
...
select District.Name DistrictName, Budgets.Name BudgetName, Requisitions.RequisitionNumber, School.Name SchoolName, Users.CometId, Requisitions.Attention, we.ErrorMsg
  from @we we
 ...
 order by District.Name, Budgets.Name, Requisitions.RequisitionId, we.ErrorMsg
```
