# Procedure: `dbo.sp_CopyMSRPVers2BidUsingCursorSave2`

_Generated on 2026-05-04T13:43:18.759Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `sp_CopyMSRPVers2BidUsingCursorSave2` |
| Kind | Procedure |
| sys.objects.type | `P` (SQL_STORED_PROCEDURE) |
| Created | 2014-01-16 19:33:41 |
...
-- =============================================
CREATE procedure [dbo].[sp_CopyMSRPVers2BidUsingCursorSave2] @pOldBidHeaderId  varchar(50), @pNewBidHeaderId  varchar(50)
AS
BEGIN
...
begin
  ...
  begin
    ...
    declare BRPRCur cursor fast_forward read_only for
      select BidRequestPriceRangeId
        from BidRequestPriceRanges
       where BidRequestProductLineId = @BidRequestProductLineId
    order by BidRequestPriceRangeId    

    open BRPRCur
    fetch next from BRPRCur into @BidRequestPriceRangeId
    while @@fetch_Status = 0 
    begin
      ...
      FROM BidRequestPriceRanges
      WHERE BidRequestPriceRangeId = @BidRequestPriceRangeId

      --select @NewBidRequestPriceRangeId = Scope_Identity() --DCH 11/24/2015 @@Identity from BidRequestPriceRanges
      --Print 'Old BidRequestPriceRangeId = ' + cast(@BidRequestPriceRangeId as Varchar(30)) + '  New BidRequestPriceRangeId = ' + cast(@NewBidRequestPriceRangeId as Varchar(30))
    ...
    declare BROptCur cursor fast_forward read_only for
      select BidRequestOptionId
        from BidRequestOptions
       where BidRequestProductLineId = @BidRequestProductLineId
    order by BidRequestOptionId    

    open BROptCur
    fetch next from BROptCur into @BidRequestOptionId
    while @@fetch_Status = 0 
    begin
      ...
      FROM BidRequestOptions
      WHERE BidRequestOptionId = @BidRequestOptionId

      --select @NewBidRequestOptionId = Scope_Identity() --DCH 11/24/2015 @@Identity from BidRequestOptions
      --Print 'Old BidRequestOptionId = ' + cast(@BidRequestOptionId as Varchar(30)) + '  New BidRequestOptionId = ' + cast(@NewBidRequestOptionId as Varchar(30))
