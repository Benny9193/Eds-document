# Function: inline table-valued: `null.uf_LookupItemsForBook1`

_Generated on 2026-05-04T13:04:00.228Z_
...
```sql
CREATE    function EDSIQWebUser.uf_LookupItemsForBook1 (@pEffectiveDate datetime, @pBookId int)
returns table 
 ...
 return (
  ...
  union (
   ...
   join dbo.PPCatalogs on PPCatalogs.PricePlanId = DistrictPP.PricePlanId
		     ...
		     where DistrictPP.DistrictId = BatchDetail.DistrictId
	               and Items.Active = 1
                               ) sCat on sCat.ItemId = Items.ItemId
            left outer join (
		    select Items.ItemId, CrBid.CrossRefId, BidItems.BidItemId, Awards.AwardId
