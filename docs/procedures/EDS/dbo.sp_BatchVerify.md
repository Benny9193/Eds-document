# Procedure: `dbo.sp_BatchVerify`

_Generated on 2026-05-04T13:07:57.351Z_
...
```sql
CREATE procedure [dbo].[sp_BatchVerify] @pBatchId int AS
...
update BatchDetail
  ...
  from BatchDetail
  inner join Category on Category.Code = BatchDetail.Category
 where BatchId = @BatchId
   and BatchDetail.Active = 1
...
update BatchDetail
   set ErrorField = 2
 where BatchId = @BatchId   and CategoryId is null   and ErrorField is null
   and BatchDetail.Active = 1
...
-- Validate Item Code using CrossRef Codes
  update BatchDetail
    ...
    join Catalog on Catalog.CatalogId = BidsCatalogList.CatalogId
                and Catalog.CrossRefLetter = BatchDetail.Location
                and Catalog.CategoryId = BatchDetail.CategoryId
                and Catalog.Active = 1
...
-- Special Validate Item Code using CrossRef Codes for Allied / Computer/Office Not Pre-Bid
  update BatchDetail
     set BatchDetail.ItemId = Items.ItemId,
         BatchDetail.BidPrice = null
    from BatchDetail
...
--                         and DistrictCategories.AllowAddenda = 1
 where BatchDetail.BatchId = @BatchId
   and isnull(BatchDetail.BidPrice,0) = 0 -- is null
...
-- Update Event
Update TaskSchedule
   set StartDateActual = isnull(StartDateActual,Batches.Loaded),
       EndDateActual = Batches.Loaded
--select BatchDetail.DistrictId, BatchDetail.CategoryId, Batches.Loaded, case isnull(TaskSchedule.StartDateActual,0) when 0 then Batches.Loaded else TaskSchedule.StartDateActual end Started, case isnull(TaskSchedule.EndDateActual,0) when 0 then Batches.Loaded else TaskSchedule.EndDateActual end Ended
  from Batches
  join BatchDetail on BatchDetail.BatchId = Batches.BatchId
  join TaskSchedule on TaskSchedule.DistrictId = BatchDetail.DistrictId
                   and TaskSchedule.CategoryId = BatchDetail.CategoryId
                   and TaskSchedule.ProjectTasksId = 4
 ...
 where Batches.BatchId = @BatchId
-- group by BatchDetail.DistrictId, BatchDetail.CategoryId, Batches.Loaded, TaskSchedule.StartDateActual, TaskSchedule.EndDateActual
