# Function: table-valued: `dbo.uf_LookupItemCodeByReqOld120912`

_Generated on 2026-05-04T13:04:24.273Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `uf_LookupItemCodeByReqOld120912` |
| Kind | Function (table-valued) |
| sys.objects.type | `TF` (SQL_TABLE_VALUED_FUNCTION) |
| Created | 2012-09-12 13:02:44 |
| Modified | 2012-09-12 13:02:44 |
| Encrypted | no |
...
create    function [dbo].[uf_LookupItemCodeByReqOld120912] (@pRequisitionId int, @pItemCode varchar(255), @pCatalogId int)
...
/*
  delete @ItemTable
    from @ItemTable it
   where convert(char(1),isnull(PriceType,-1) + 1) + 
         convert(char(1),isnull(ItemBidType,3)) + right('0000000000000000' + convert(varchar(16),isnull(BidItemId,0)),16)
         < 
         (select top 1 convert(char(1),isnull(PriceType,-1) + 1) + 
                       convert(char(1),isnull(ItemBidType,3)) + right('0000000000000000' + convert(varchar(16),isnull(BidItemId,0)),16)
            from @ItemTable it1 
            join CrossRefs on CrossRefs.CrossRefId = it1.CrossRefId 
           where CrossRefs.ItemId = (select top 1 x1.ItemId 
                                       from CrossRefs x1 with (nolock) 
                                      where x1.CrossRefId = it.CrossRefId) 
           order by convert(char(1),isnull(PriceType,-1) + 1) + 
                    convert(char(1),isnull(ItemBidType,3)) + right('0000000000000000' + convert(varchar(16),isnull(BidItemId,0)),16) desc)
*/
