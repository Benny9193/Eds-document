# Function: table-valued: `dbo.uf_SavingsLetter`

_Generated on 2026-05-04T14:49:07.416Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `uf_SavingsLetter` |
| Kind | Function (table-valued) |
| sys.objects.type | `TF` (SQL_TABLE_VALUED_FUNCTION) |
| Created | 2007-04-16 14:57:50 |
| Modified | 2012-02-09 15:32:37 |
| Encrypted | no |
...
CREATE      function [dbo].[uf_SavingsLetter] (@pRSId int, @PassDate varchar(255)) 
...
select DistrictId, CategoryId, isnull(OnSavingsReport,0), DistrictName, CategoryName, 
  ...
  from (
    select District.DistrictId, Category.CategoryId, isnull(Category.OnSavingsReport,0) OnSavingsReport,
           District.Name + 
             case patindex('%PAYABLE%',isnull(BAC.Address1,'')) 
               when 0 then 
                 case rtrim(isnull(BAC.Address1,'')) 
                   when '' then '' 
                   else char(13) + char(10) + BAC.Address1 
                 end 
               else '' 
             end + 
             case patindex('%PAYABLE%',isnull(BAC.Address2,'')) 
               when 0 then 
                 case rtrim(isnull(BAC.Address2,'')) 
                   when '' then '' 
                   else char(13) + char(10) + BAC.Address2 
                 end 
               else '' 
             end + 
             char(13) + char(10) + isnull(BAC.City,'') + ', ' + isnull(BAC.State,'') + '  ' + isnull(BAC.Zipcode,'') DistrictName, 
...
--       and District.DistrictId = 34 
     ...
     group by District.DistrictId, Category.CategoryId, Category.OnSavingsReport,
              District.Name, BAC.Address1, BAC.Address2, 
              BAC.City, BAC.State, BAC.Zipcode, Category.Name, 
