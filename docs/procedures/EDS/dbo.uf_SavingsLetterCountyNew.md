# Function: table-valued: `dbo.uf_SavingsLetterCountyNew`

_Generated on 2026-05-04T13:07:57.706Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `uf_SavingsLetterCountyNew` |
| Kind | Function (table-valued) |
| sys.objects.type | `TF` (SQL_TABLE_VALUED_FUNCTION) |
| Created | 2008-11-12 11:16:51 |
...
```sql
CREATE       function dbo.uf_SavingsLetterCountyNew (@pRSId int, @PassDate varchar(255)) 
...
--       and Detail.BidItemId is not null
...
Update @CatTable
   set IncludedSavings = (select sum(TotalSavings) from @CatTable cts where cts.DistrictId = ct.DistrictId),
       IncludedDiscount = 1 - ((select sum(TotalBidCost) from @CatTable cts where cts.DistrictId = ct.DistrictId) / (select sum(TotalCatalogCost) from @CatTable cts where cts.DistrictId = ct.DistrictId)),
...
select 0 DistrictId, isnull(ct1.CategoryId,0) CategoryId, isnull(Category.Name,'') CategoryName
