# Procedure: `dbo.sp_PostCatalog6`

_Generated on 2026-05-04T13:07:58.734Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `sp_PostCatalog6` |
| Kind | Procedure |
| sys.objects.type | `P` (SQL_STORED_PROCEDURE) |
| Created | 2026-02-26 21:08:21 |
| Modified | 2026-02-26 21:08:21 |
| Encrypted | no |
...
```sql
create   PROCEDURE [dbo].[sp_PostCatalog6] @pCatalogId int as
...
begin
-- ============================================================================
-- ImportFormat 6: UOM-aware cross-ref matching (like Format 5) + UOM suffix
-- ============================================================================

-- Match Existing CrossRefs (UOM-aware)
Update [Master Catalog]
   set ItemId = CrossRefs.ItemId
  from [Master Catalog] mc
  join eds.dbo.Catalog Catalog on Catalog.CatalogId = mc.CatalogId
...
INSERT INTO [EDS].[dbo].[PostCatalogDetail](PostCatalogHeaderId, PostInfoType, PostInfoDesc, PostInfoValue, PostDateTime)
     VALUES(@PostCatalogHeaderId, 2, 'Match Existing Cross References', @@RowCount, getdate())
/* I don't think we want to do this - but I think we have to */
-- Suffix UniqueItemNumber for UOM variants (items not matched because UOM differs)
UPDATE mc
   SET UniqueItemNumber = mc.UniqueItemNumber + '-' + ltrim(rtrim(mc.UnitCode))
  FROM [Master Catalog] mc
  JOIN eds.dbo.Catalog Catalog ON Catalog.CatalogId = mc.CatalogId
  JOIN eds.dbo.Items Items ON Items.CategoryId = Catalog.CategoryId
  ...
  JOIN eds.dbo.Units Units ON Units.UnitId = Items.UnitId
 WHERE mc.CatalogId = @CatalogId
   AND mc.ItemId IS NULL
   AND ltrim(rtrim(Units.Code)) != isnull(ltrim(rtrim(mc.UnitCode)), 'EACH')

INSERT INTO [EDS].[dbo].[PostCatalogDetail](PostCatalogHeaderId, PostInfoType, PostInfoDesc, PostInfoValue, PostDateTime)
     VALUES(@PostCatalogHeaderId, 14, 'Detect UOM Changes - New UOM Items', @@RowCount, getdate())
...
begin
-- ============================================================================
-- ImportFormat 6: Create & Match Items (UniqueItemNumber already suffixed for UOM variants)
-- ============================================================================
...
-- Match Newly Created Items (UOM-aware)
Update [Master Catalog]
   set ItemId = Items.ItemId
  from [Master Catalog] mc
  join eds.dbo.Catalog Catalog on Catalog.CatalogId = mc.CatalogId
...
begin
-- ============================================================================
-- ImportFormat 6: Create & Match CrossRefs (UniqueItemNumber includes UOM suffix)
-- ============================================================================
