# Procedure: `dbo.sp_PostCatalogOrig`

_Generated on 2026-05-04T13:07:58.740Z_
...
|----------|-------|
| Schema | `dbo` |
| Name | `sp_PostCatalogOrig` |
| Kind | Procedure |
| sys.objects.type | `P` (SQL_STORED_PROCEDURE) |
| Created | 2023-02-02 17:04:50 |
| Modified | 2023-02-02 17:04:50 |
| Encrypted | no |
...
```sql
create   PROCEDURE [dbo].[sp_PostCatalogOrig] @pCatalogId int as
...
begin
Update [Master Catalog]
   set ItemId = xr.ItemId
--select *
  from [Master Catalog] mc
  join eds.dbo.Catalog Catalog on Catalog.CatalogId = mc.CatalogId
  outer apply (Select top 1 CrossRefs.ItemId
                 from eds.dbo.CrossRefs
				 join eds.dbo.Items on Items.ItemId = CrossRefs.ItemId
				                   and Items.Active = 1
				 join eds.dbo.Catalog cat on cat.CatalogId = CrossRefs.CatalogId
										 and cat.CategoryId = Catalog.CategoryId
										 and cat.VendorId = Catalog.VendorId
										 and cat.Active = 1
										 and cat.CatalogId != Catalog.CatalogId
				where coalesce(trim(CrossRefs.UniqueItemNumber),'') = coalesce(trim(mc.UniqueItemNumber),'')
				  and CrossRefs.Active = 1
				order by cat.CatalogYear desc, cat.CatalogId desc, CrossRefs.CrossRefId desc) xr
 where mc.CrossRefId is null
   and mc.CatalogId = @CatalogId
   and xr.ItemId is not null
/* Old Code Repplaced 2/2/2023 by DCH with Code Above
Update [Master Catalog]
...
Update [Master Catalog]
  ...
  from [Master Catalog] mc
  join eds.dbo.Catalog Catalog on Catalog.CatalogId = mc.CatalogId
  outer apply (Select top 1 Items.ItemId
                 from eds.dbo.Items
				where Items.CategoryId = Catalog.CategoryId
				  and Items.Active = 1
				  and (   (     Items.PackedCode = coalesce(Catalog.Prefix,'') + mc.PackedCode
							and coalesce(trim(Items.ManufacturorNumber),'') = coalesce(trim(mc.ManufacturorPartNumber),'') 
							and coalesce(trim(Items.Description),'') = coalesce(trim(mc.Description),''))
					   or (   Items.ItemCode = coalesce(Catalog.Prefix,'') + coalesce(trim(mc.UniqueItemNumber),'')))
				order by case when Items.ItemCode = coalesce(Catalog.Prefix,'') + coalesce(trim(mc.UniqueItemNumber),'') then 1 else 0 end +
				         case when coalesce(trim(Items.ManufacturorNumber),'') = coalesce(trim(mc.ManufacturorPartNumber),'') then 1 else 0 end +
						 case when coalesce(trim(Items.Description),'') = coalesce(trim(mc.Description),'') then 1 else 0 end desc, Items.ItemId desc) i
