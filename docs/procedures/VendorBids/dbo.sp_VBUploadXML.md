# Procedure: `dbo.sp_VBUploadXML`

_Generated on 2026-05-04T13:08:01.425Z_
...
| Created | 2011-09-20 17:29:38 |
| Modified | 2022-08-12 10:58:46 |
| Encrypted | no |
...
-- =============================================
CREATE PROCEDURE [dbo].[sp_VBUploadXML]
...
--  		                  and DownloadLog.calendarid = @pBidId
	...
	begin
		...
		begin
			...
			if @CategoryType = 5
			begin
			  insert into VendorBidMSRPResults (VendorBidId, BidRequestManufacturerId, ManufacturerName, WriteInManufacturer, PriceListType, TotalAwardDiscount, BidRequestProductLineId, ProductLineName, WriteInProductLine, BidRequestOptionId, OptionName)
			    select VendorBids.vendorbidid, x.BidRequestManufacturerId, x.ManufacturerName, x.WriteInManufacturer, x.PriceListType, EncryptByPassPhrase(@pPassPhrase, x.TotalAwardDiscount, 1, cast(VendorBids.vendorbidid as varbinary)), x.BidRequestProductLineId, x.ProductLineName, x.WriteInProductLine, x.BidRequestOptionId, x.OptionName
				  ...
				  with (BidRequestManufacturerId varchar(50) '../../../BidRequestManufacturerId',
						...
						TotalAwardDiscount varchar(512) '../../../TotalAwardDiscount',
				...
				insert vendorbiditems (vendorbidid, bidrequestitemid, quantity, itemid)
				  select vendorbids.vendorbidid, CAST(x.BidRequestItemId as int), CAST(x.QuantityBid as int), 1
					...
					left outer join vendorbiditems on vendorbiditems.vendorbidid = vendorbids.vendorbidid
												  and coalesce(vendorBidItems.BidRequestItemId_OLD, vendorbiditems.bidrequestitemid) = CAST(x.BidRequestItemId as int)
				   where vendorbids.vendorbidid = @VendorBidId
					 and vendorbiditems.vendorbiditemid is null                                      

				-- add journal entry
				insert vendorbiditemsjournal (vendorbiditemid, sessionid, itembidtype, unitprice, cost, vendoritemcode, quantitybid, alternate, itemsperunit, pageno, Manufacturer, ManufacturerPartNumber, LinerGaugeMicrons, LinerGaugeMil, LinerCaseWeight, LinerDimWidth, LinerDimDepth, LinerDimLength, SDS_URL, ImageURL, UPC_ISBN, UniqueItemNumber)
				  select vendorbiditems.vendorbiditemid,
						 ...
						 EncryptByPassPhrase(@pPassPhrase, x.SDS_URL, 1, cast(vendorbiditems.vendorBidItemId as varbinary)),
						 EncryptByPassPhrase(@pPassPhrase, x.ImageURL, 1, cast(vendorbiditems.vendorBidItemId as varbinary)),
						 EncryptByPassPhrase(@pPassPhrase, x.UPC_ISBN, 1, cast(vendorbiditems.vendorBidItemId as varbinary)),
						 EncryptByPassPhrase(@pPassPhrase, x.UniqueItemNumber, 1, cast(vendorbiditems.vendorBidItemId as varbinary))
 					from OPENXML(@hDoc, '/BidResults/BidResultDetail') 
					with (BidRequestItemId varchar(50) 'BidRequestItemId',
				 		  Alternate varchar(512) 'Alternate',
						  ItemBidType varchar(50) 'ItemBidType',
						  ...
						  LinerDimLength varchar(50) 'LinerDimLength',
						  SDS_URL varchar(300) 'SDS_URL',
						  ImageURL varchar(300) 'ImageURL',
						  UPC_ISBN varchar(50) 'UPC_ISBN',
						  UniqueItemNumber varchar(50) 'UniqueItemNumber') x
					join vendorbiditems on coalesce(VendorBidItems.BidRequestItemId_Old, vendorbiditems.bidrequestitemid) = cast(x.BidRequestItemId as int)
