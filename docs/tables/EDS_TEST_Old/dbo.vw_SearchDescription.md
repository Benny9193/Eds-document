...
AS  
/*with cte_bi(BidHeaderId, ItemId, BidItemId, ItemBidType, Alternate, ManufacturerBid, ManufPartNoBid, VendorItemCode, FullDescription, ShortDescription) as (
Select Bids.BidHeaderId, BidItems.ItemId, BidItems.BidItemId, BidItems.ItemBidType, BidItems.Alternate, BidResults.ManufacturerBid, BidResults.ManufPartNoBid, BidResults.VendorItemCode, coalesce(xr.AIFullDesc,xr.FullDescription) FullDescription, coalesce(xr.AIShortDesc,xr.ShortDescription) ShortDescription
  from dbo.BidItems
...
/*										and bmi.BidMappedItemId = (Select top 1 BidMappedItems.BidMappedItemId
																	 from BidMappedItems
																	where BidMappedItems.BidHeaderId = BidHeaders.BidHeaderId
																	  and BidMappedItems.OrigItemId = Items.ItemId
																	order by BidMappedItems.MapDate desc, BidMappedItems.BidMappedItemId desc) 
*/										     
...
*/
	  left outer join BidItems on BidItems.biditemId = (select top 1 BidItems.BidItemId
														 from dbo.BidItems
														 join dbo.Bids on Bids.BidId = BidItems.BidId
																  and Bids.Active = 1
														 join dbo.BidResults on BidResults.BidResultsId = BidItems.BidResultsId
														 join dbo.BidHeaders bh on bh.BidHeaderId = Bids.BidHeaderId
																		   and bh.BidHeaderId = BidHeaders.BidHeaderId
																		   and bh.Active = 1
														 left outer join dbo.CrossRefs xr on BidItems.CrossRefId = BidItems.CrossRefId
...
*/	  left outer join CrossRefs cxc on cxc.ItemId = coalesce(bmi.NewItemId, Items.ItemId)
	                               and cxc.CatalogId in (Select BidsCatalogList.CatalogId
								                           from BidsCatalogList
														   join Bids on Bids.BidId = BidsCatalogList.BidId
														            and Bids.Active = 1
																	and Bids.BidHeaderId = BidHeaders.BidHeaderId)
								   and cxc.Active = 1
	  outer apply (select coalesce(cxc.AIFullDesc,cxc.FullDescription) FullDescription, coalesce(cxc.AIShortDesc,cxc.ShortDescription) ShortDescription) cxr
