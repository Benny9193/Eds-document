...
District notification routing rules (~6K rows). Per (`DistrictId`, `NotificationType`, optional `CategoryId` / `UserId`) — defines `Method`, `NotifyList`, and `OtherNotify` so the system knows where to send each event class for that district.
