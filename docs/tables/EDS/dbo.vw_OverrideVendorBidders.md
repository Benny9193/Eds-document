  ...
  when 0 then 1 
    ...
    case isnull(BidResults.Active,0) 
      when 0 then 1 
		...
		case 
		  when isnull(BidResults.ItemBidType,'') in ('C','S') then 0
		  else 1
		end
    end 
end as CHAR(1)) +
right(replicate('0',14) + cast(convert(decimal(14,2),(isnull(BidResults.UnitPrice,0) - ((isnull(BidImports.BidItemDiscountRate,0) * isnull(BidResults.UnitPrice,0)) / 100))) as varchar),14) +
