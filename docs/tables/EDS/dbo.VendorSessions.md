...
Vendor-portal session tracking (~11K rows). One row per vendor login session — `VendorId`, `UserName`, `jSession` token, `StartTime` / `EndTime`, `IPAddress`, and `VPORegistrationId`. Distinct from `Users` (district-side) sessions.
