...
Vendor bid-response import header (~56K rows). One row per (`BidHeaderId`, `VendorId`) captured from a vendor's submitted bid file, with `BidItemDiscountRate`, `CatalogDiscountRate`, `MinimumOrder`, free-delivery / free-handling terms, and `Status`. Pairs with `Bids` (response header) and `BidResults` (line items).
