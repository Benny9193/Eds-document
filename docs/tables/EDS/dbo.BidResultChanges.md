...
Append-only edit log on `BidResults` (~18.2M rows). Records before/after `Active`, `UnitPrice`, `BidType`, and `Comments` whenever a vendor's bid response is modified. Pairs with `BidResultsChangeLog` which adds the user / reason context.
