...
|---|--------|------|----------|---------|----|-------------|
| 1 | `DetailId` | int | NO |  | YES | Surrogate primary key. Stable across the requisition → PO conversion; line items keep the same `DetailId` once they migrate from req-only to PO-attached. |
| 2 | `RequisitionId` | int | YES |  |  | FK to `Requisitions`. Always set — every line item belongs to a requisition. Combined with `RequisitionId`-based filtering for any per-req query. |
| 3 | `CatalogId` | int | YES |  |  |  |
| 4 | `ItemId` | int | YES |  |  | FK to `Items` (the master product). NULL for free-text / off-catalog lines. When set, prefer joining via `CrossRefId` to get vendor-specific pricing. |
| 5 | `AddendumItem` | tinyint | YES |  |  |  |
| 6 | `ItemCode` | varchar(50) | YES |  |  |  |
| 7 | `Quantity` | int | YES |  |  | Units ordered. Integer; fractional quantities are not supported on this table. Combined with `BidPrice` to compute line total. |
| 8 | `LastYearsQuantity` | int | YES |  |  |  |
| 9 | `Description` | varchar(1024) | YES |  |  |  |
| 10 | `UnitId` | int | YES |  |  |  |
| 11 | `UnitCode` | varchar(20) | YES |  |  |  |
| 12 | `BidPrice` | money | YES |  |  | Per-unit price at requisition time, in dollars. The frozen price the requisition is being approved against; line total = `BidPrice * Quantity`. Independent of the live `CrossRefs.CatalogPrice` (which can change). |
...
| 18 | `PriceId` | int | YES |  |  |  |
| 19 | `AwardId` | int | YES |  |  |  |
| 20 | `VendorId` | int | YES |  |  | FK to `Vendors`. The vendor this line is being purchased from — denormalized from `CrossRefs` for performance. |
| 21 | `VendorItemCode` | varchar(50) | YES |  |  |  |
| 22 | `Alternate` | varchar(1024) | YES |  |  |  |
| 23 | `POId` | int | YES |  |  | FK to `PO`. NULL while the line is still on an unconverted requisition; set when the requisition reaches final approval and converts to a purchase order. Use `WHERE POId IS NULL` to find req-only lines. |
...
| 33 | `BidHeaderId` | int | YES |  |  | FK to `BidHeaders` — which bid award this price came from. Important for spend-by-bid reporting and for verifying that the captured price still reflects an active award. |
...
| 44 | `Active` | tinyint | YES |  |  | Tinyint flag (0/1). 0 marks a line as soft-deleted / cancelled; rows are rarely physically removed because of audit references in `DetailChangeLog`. |
| 45 | `RTK_MSDSId` | int | YES |  |  |  |
| 46 | `AddedFromAddenda` | datetime | YES |  |  |  |
| 47 | `LastAlteredSessionId` | int | YES |  |  |  |
| 48 | `AdditionalShipping` | tinyint | YES |  |  |  |
| 49 | `CrossRefId` | int | YES |  |  | FK to `CrossRefs` — the specific vendor-priced row this line was built from. Captures the vendor and price at requisition time. NULL for free-text lines. |
