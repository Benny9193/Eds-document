...
Per-district vendor enablement (~317K rows). FK to both `District` and `Vendors`; the `Active` flag scopes a vendor's visibility to a specific district, and `VendorsAccountCode` stores the district's internal account code with that vendor (used on POs).
