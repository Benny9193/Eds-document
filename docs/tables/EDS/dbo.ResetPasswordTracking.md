...
Password-reset email audit (~125K rows). One row per reset request with the issued `ResetPasswordCode`, target email, district / school context, and action / error payload. Indexed on `ResetPasswordCode` for verification.
