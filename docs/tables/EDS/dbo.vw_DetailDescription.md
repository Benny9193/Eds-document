...
-- Replaced by Previous Line				   dbo.uf_DetailItemDescriptionNoExtraNH(Requisitions.RequisitionId, Detail.ItemId)
	  ...
	  outer apply (  
    select coalesce(case when coalesce(BidResults.ItemBidType,'') in ('S', 'C') and coalesce(trim(BidResults.AIShortDesc),'') != '' then coalesce(trim(BidResults.AIShortDesc),'') else null end,
