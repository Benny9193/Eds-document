...
District contact directory (~3.8K rows). Named contacts of a given `DistrictContactTypeId` per district — full name parts, salutation, phone / fax / email, and a postal address (which can be linked to a `ShippingId`). Used for billing, RFP, and notification routing.
