...
Junction `PricePlans` × `Category` × `Catalog` (~1.7K rows). Determines which catalogs are visible under a price plan for a category, with the `DiscountRate` and the linking `AwardId`. The price-plan side of catalog visibility (vs `BidsCatalogList` / `AwardsCatalogList` which are bid-side).
