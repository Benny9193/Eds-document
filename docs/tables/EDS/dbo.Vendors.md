...
| 2 | `Active` | tinyint | YES |  |  | Tinyint flag (0/1) controlling whether the vendor is visible across the platform. Setting to 0 hides the vendor's catalogs and bid responses everywhere — system-wide effect, not a soft delete. |
| 3 | `Code` | varchar(16) | YES |  |  | Short alphanumeric vendor code (≤16 chars) used as a human-friendly identifier on POs and reports. Distinct from `VendorId` (the surrogate key). |
...
| 14 | `UseGrossPrices` | tinyint | YES |  |  | Tinyint flag (0/1). If 1, this vendor's pricing is interpreted as gross (list) prices that must be discounted against bid terms. If 0, prices are net. |
| 15 | `ShippingPercentage` | decimal(9,5) | YES |  |  |  |
| 16 | `DistrictId` | int | YES |  |  | When non-null, marks this vendor as district-specific (only visible to that district). When null, the vendor is platform-wide. |
