# View: `dbo.InstructionBookViewCF2013`
...
CREATE     view  [dbo].[InstructionBookViewCF2013]   AS
/*
...
select District.DistrictId,
       ...
       right('00000' + cast(Users.CometId as varchar),5) CometId,        min(Accounts.Code) AccountCode, 
 ...
 case rtrim(isnull(CSRep.Name,'')) when '' then '' else 'If you need additional assistance your Ed-Data Customer Service Representative is ' + rtrim(isnull(CSRep.Name,'not yet assigned')) + '<br/>and can be reached at ' end +
 'Phone: ' + rtrim(isnull(CSRep.Phone,'(973) 340-8800')) + 
 case rtrim(isnull(CSRep.Email,'')) when '' then '' else ' or Email: ' + rtrim(CSRep.EMail) end as RepMsg,
 ...
 ISNULL(Users.AllowAddenda,0) AllowAddenda,
 case when ISNULL(Users.AllowAddenda,0) | ISNULL(Users.AllowVendorChanges,0) > 0 then 1 else 0 end AllowVendorChanges,
 isnull(Budgets.BudgetId,0) BudgetId,
 isnull(Users.AllowAccountCodeMgmt,0) AllowAccountCodeMgmt,
 isnull(Users.HasAdminAccess,0) HasAdminAccess
...
*/	  else
	    case ISNULL(Users.ApprovalLevel,0)
	      when 1 then 2
	      when 2 then 6
	      else
		    2
 ...
 group by District.DistrictId,
       ...
       Users.AllowAddenda,
	   Users.AllowVendorChanges,
	   Budgets.BudgetId,
	   isnull(Users.AllowAccountCodeMgmt,0),
	   isnull(Users.HasAdminAccess,0)
