...
Master image-asset record (~1.7M rows). Holds source URL, local paths (full / resized / thumbnail), perceptual hash (`pHash`, `bipHash`) for dedup, format / dimensions, and a `dateDeleted` soft-delete column. The single source of truth for product imagery referenced by catalog UI.
