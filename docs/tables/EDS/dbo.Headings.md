...
Sub-category browse-tree headings under each `Category` (~306K rows). Can be district-scoped via `DistrictId`; `ExpandAll` controls default UI expansion. Drives the catalog browse hierarchy buyers see when shopping.
...
| `SKI_ActiveTitle` | no | NONCLUSTERED | `Active`, `Title` |  |
