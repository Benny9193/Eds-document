...
Proposed-but-unapplied district charges (~12K rows). Pre-application staging row capturing the proposed `Amount`, `PreviousAmount`, `ChangePercentage`, `Action`, and timing — moves into `DistrictCharges` when applied (`DateApplied` set).
