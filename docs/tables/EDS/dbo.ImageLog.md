...
Image-fetch audit log (~1.8M rows). One row per HTTP fetch attempt with `statusCode`, `statusText`, `contentType`, response headers, and write outcome. Used to diagnose broken / stale image URLs sourced from vendor catalogs.
