# Table: `dbo.MSRPExcelImport`

**Database:** `EDS` &nbsp;|&nbsp; **Schema:** `dbo`
**Approx rows:** 76315

[← back to database index](README.md) &nbsp;|&nbsp; [← back to top](../../../SCHEMA.md)

## Description

Staging table for vendor-submitted MSRP price-list Excel uploads (~76K rows). Holds parsed rows from the spreadsheet (manufacturer, product-line group, delivery option, and up to seven (Base, Weight) tiers) before promotion into `BidMSRPResults*` / `PriceRanges`.

## Columns

| # | Column | Type | Nullable | Default | PK |
|---|--------|------|----------|---------|----|
| 1 | `MSRPExcelImportId` | int | NO |  | YES |
| 2 | `BidNumber` | int | YES |  |  |
| 3 | `SeqNumber` | int | YES |  |  |
| 4 | `Manufacturer` | varchar(100) | YES |  |  |
| 5 | `ProductLineGroup` | varchar(100) | YES |  |  |
| 6 | `DeliveryOption` | varchar(50) | YES |  |  |
| 7 | `ManufWebsite` | varchar(255) | YES |  |  |
| 8 | `Base1` | money | YES |  |  |
| 9 | `Weight1` | decimal(9,5) | YES |  |  |
| 10 | `Base2` | money | YES |  |  |
| 11 | `Weight2` | decimal(9,5) | YES |  |  |
| 12 | `Base3` | money | YES |  |  |
| 13 | `Weight3` | decimal(9,5) | YES |  |  |
| 14 | `Base4` | money | YES |  |  |
| 15 | `Weight4` | decimal(9,5) | YES |  |  |
| 16 | `Base5` | money | YES |  |  |
| 17 | `Weight5` | decimal(9,5) | YES |  |  |
| 18 | `Base6` | money | YES |  |  |
| 19 | `Weight6` | decimal(9,5) | YES |  |  |
| 20 | `Base7` | money | YES |  |  |
| 21 | `Weight7` | decimal(9,5) | YES |  |  |
| 22 | `Base8` | money | YES |  |  |
| 23 | `Weight8` | decimal(9,5) | YES |  |  |
| 24 | `Base9` | money | YES |  |  |
| 25 | `Weight9` | decimal(9,5) | YES |  |  |
| 26 | `Base10` | money | YES |  |  |
| 27 | `Weight10` | decimal(9,5) | YES |  |  |

## Foreign keys (outgoing)

_None._

## Referenced by (incoming foreign keys)

_None._

## Indexes

_No non-PK indexes._
