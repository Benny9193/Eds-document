...
Catalog / data-import job header (~301 rows). One row per import attempt — `ImportType`, `ImportDate`, `Records`, `ErrorCount`, target `CategoryId`, `PricePlanId`, and up to six `CatalogId1`–`CatalogId6` slots being loaded simultaneously. Companion to `Batches` (which is requisition-side).
