...
Legacy NJ-RTK school-file mapping (~6.8K rows). Maps the old (`LegacyDistrictCode`, `LegacySchoolCode`, `NJEIN`) identifiers + `ExposedEmployees` count to current `DistrictId` / `RTK_SitesId`. Read-mostly; preserved so historical RTK exports can be reproduced.
