...
Saved email-blast definitions / send history (~18K rows). One row per blast — name, HTML body, recipient `SQLStmt` and `ReportWhereClause`, `EmailFrom`/`CC`/`BCC`/`Subject`, attachments, priority and read-receipt flags, and `SentDate` once dispatched.
