...
Denormalized flat snapshot joining `BidHeaders` × `Items` × `CrossRefs` × `Catalog` × `PricePlans` × `Category` (~6.3M rows). Reporting / working table — pricing, descriptions, vendor item codes, and order-vs-bid totals collapsed into one wide row. Rebuilt by batch jobs; not a system of record.
