...
Per-product-line MSRP bid response (~110K rows). Lines under `BidMSRPResults` with the vendor's per-line `WeightedDiscount`, `MSRPOptionId`, and write-in product-line text. Resolves the actual rate applied for each product line in a vendor's MSRP response.
