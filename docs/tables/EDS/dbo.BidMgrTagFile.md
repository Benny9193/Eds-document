...
Bid-manager tagging working set (~4.4M rows). Per-user (`Usr`) tag overlay against a target table (`Tbl`) and row pointer (`Ptr`) with current and original tag values. Used by the bid-management UI to track in-progress tag changes before persistence.
