...
Master list of bidder-checklist items (~140 rows). Per-checklist item: `CheckListText`, optional rich-text help, document-name / type, and flags for `OptionalDocument`, `OnFileEligible`, `UploadEligible`, `ExpirationDateReqd`, `DocNumberReqd`, `DocNumberLabel`. `BidHeaderCheckList` is the per-bid junction.
