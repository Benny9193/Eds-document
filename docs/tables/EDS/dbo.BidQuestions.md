...
Bid question/answer template (~24K rows). One row per question on a `BidTrade`, with display geometry (X/Y/Width/Height for both question and answer fields), text, expected `QuestionQty` / `QuestionUOMId`, and `AnswerTypeId` / `AnswerTypeMask`. Drives the bid form vendors fill out; responses land in `BidAnswers`.
