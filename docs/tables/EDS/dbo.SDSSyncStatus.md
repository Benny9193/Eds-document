...
SDS-to-search-index sync status per `SafetyDataSheet` (~26K rows). Tracks `SyncedItems` / `SyncedRequisitions` against totals, separate item / requisition statuses, and start / end timestamps. Drives the SDS Elasticsearch backfill.
