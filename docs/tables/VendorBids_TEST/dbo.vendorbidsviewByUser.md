...
| 33 | `regUserId` | int | NO |  |  |
| 34 | `ShowBidTabs` | int | NO |  |  |
| 35 | `CategoryId` | int | YES |  |  |
...
| Object | Type |
|--------|------|
| [`dbo.filterCategories`](dbo.filterCategories.md) | VIEW |
| [`dbo.filterStates`](dbo.filterStates.md) | VIEW |
| [`dbo.filterStatuses`](dbo.filterStatuses.md) | VIEW |

## Definition

```sql
--select * from RegUsers join Registrations on registrations.registrationid = regusers.registrationid where registrations.Code = 'ED06'

--select * from vendorbidsviewByUser where reguserid = 741 order by calendarid

CREATE VIEW [dbo].[vendorbidsviewByUser] AS 
 SELECT cast((cast(BidScheduleView.BSCId as bigint) * 0x10000000) + (isnull(cast(bidcalendar.calendarid as bigint),0) * 524288)  + isnull(cast(vendorbids.vendorbidid as bigint),0) as bigint) as pk_vbvid, 
        ...
        isnull(BidScheduleView.CategoryName,'') + case when isnull(ltrim(BidCalendar.description),'') != '' then ' - ' + ltrim(rtrim(BidCalendar.Description)) else '' end as categoryname, 
...
/*               0x8000 = 'Not Available'
        ...
        isnull(case 
               ...
               regusers.reguserid regUserId,
        case ISNULL(registrations.ShowBidTabs,0) when 0 then case ISNULL(regcalendar.ShowBidTabs,0) when 0 then 0 else case when BidScheduleView.OpeningDate < getdate() and isnull(BidCalendar.calendarid,0) != 0 then 1 else 0 end end else case when BidScheduleView.OpeningDate < getdate() and isnull(BidCalendar.calendarid,0) != 0 then 1 else 0 end end ShowBidTabs,
        BidScheduleView.CategoryId
...
-- removed 9-30-11 dch                              and bidcalendar.calendarid = regcalendar.calendarid -- dch added 9/14/11
   left outer JOIN vendorbids ON vendorbids.vendorbidid =
     (select top 1 VendorBidId 
        from VendorBids vb with (nolock)
       where vb.calendarid = bidcalendar.calendarid
         and vb.registrationid = regcalendar.registrationid
       order by vb.created desc)
